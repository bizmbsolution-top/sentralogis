-- Migration 20260818_secure_driver_identity_rpc.sql
-- Fixes critical security vulnerabilities in Global Driver Identity RPCs.
-- Enforces auth.uid() based tenant isolation directly inside the RPC.

BEGIN;

-- 1. Helper Functions for Internal Authorization

-- Check if current authenticated user has access to a specific tenant
...
AS $$
  SELECT 
    ...
    OR EXISTS (
      SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND tenant_id = p_tenant_id AND is_active = true
    ) OR EXISTS (
      SELECT 1 FROM public.tenants WHERE user_id = auth.uid() AND id = p_tenant_id
    );
$$;

-- Check if current authenticated user is a superadmin
...
AS $$
  SELECT 
    current_setting('request.jwt.claims', true)::json->>'role' = 'service_role'
    OR EXISTS (
      SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role IN ('superadmin', 'owner')
    ) OR EXISTS (
      SELECT 1 FROM public.tenant_users WHERE user_id = auth.uid() AND role_code IN ('tenant_superadmin', 'superadmin', 'owner') AND is_active = true
    );
$$;

-- 2. Add constraints for global identity integrity
ALTER TABLE public.driver_profiles DROP CONSTRAINT IF EXISTS uq_driver_profiles_phone;
ALTER TABLE public.driver_profiles ADD CONSTRAINT uq_driver_profiles_phone UNIQUE (phone);
...
BEGIN
  ...
  -- Concurrency-safe Profile Creation/Resolution
