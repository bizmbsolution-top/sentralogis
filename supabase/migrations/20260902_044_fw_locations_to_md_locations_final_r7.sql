-- ============================================================================
-- SENTRALOGIS DATA-4E-R7 FORENSIC REPAIR
-- STATUS: NOT AUTHORIZED FOR EXECUTION
-- DO NOT DEPLOY
-- Architecture: ADR-075 fw_locations Migration
-- ============================================================================
-- R7 REPAIR OBJECTIVES:
-- R7-01: True Idempotency (legacy/canonical state classification)
-- R7-02: Canonical Collision Semantics (explicit CASE classification)
-- R7-03: Old FK Catalog Verification (exact metadata)
-- R7-04: New FK Catalog Verification (exact metadata)
-- R7-05: ON DELETE equivalence (preserve RESTRICT)
-- R7-06: ON UPDATE equivalence (preserve NO ACTION)
--
-- CRITICAL SCHEMA FINDINGS:
-- 1. fw_locations.tenant_id EXISTS (added by migration 024)
-- 2. fw_order_headers.tenant_id EXISTS (migration 024)
-- 3. fw_legs.tenant_id EXISTS (migration 024)
-- 4. md_locations has NO unique constraint on (tenant_id, external_code)
-- 5. All four legacy FKs use ON DELETE RESTRICT
-- 6. All four FK columns are UUID type, matching md_locations.id
-- ============================================================================
...
-- Step 2d: Verify semantic equivalence for reused records
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM public.fw_locations fl
    JOIN public.md_locations ml ON ml.tenant_id = fl.tenant_id AND ml.external_code = fl.location_id::TEXT
    WHERE ml.name != fl.name OR ml.location_type != fl.type
  ) THEN
...
-- ============================================================================
-- PHASE 5: POST-TRANSFORMATION VALIDATION
-- ============================================================================
...
-- Zero remaining legacy FK references (4/4)
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.fw_order_headers oh JOIN public.fw_locations fl ON oh.origin_port_id = fl.location_id WHERE oh.tenant_id = fl.tenant_id) THEN
    RAISE EXCEPTION 'fw_order_headers.origin_port_id still has legacy references';
  END IF;
  IF EXISTS (SELECT 1 FROM public.fw_order_headers oh JOIN public.fw_locations fl ON oh.dest_port_id = fl.location_id WHERE oh.tenant_id = fl.tenant_id) THEN
    RAISE EXCEPTION 'fw_order_headers.dest_port_id still has legacy references';
  END IF;
  IF EXISTS (SELECT 1 FROM public.fw_legs leg JOIN public.fw_locations fl ON leg.start_location_id = fl.location_id WHERE leg.tenant_id = fl.tenant_id) THEN
    RAISE EXCEPTION 'fw_legs.start_location_id still has legacy references';
  END IF;
  IF EXISTS (SELECT 1 FROM public.fw_legs leg JOIN public.fw_locations fl ON leg.end_location_id = fl.location_id WHERE leg.tenant_id = fl.tenant_id) THEN
