...
BEGIN
    ...
    FOR v_src_item IN SELECT * FROM wh_repacking_items WHERE repacking_order_id = p_order_id AND item_type = 'SOURCE' LOOP
        ...
        FOR v_source_inv IN SELECT * FROM wh_inventory WHERE tenant_id = v_order.tenant_id AND warehouse_id = v_order.warehouse_id AND product_sku_id = v_src_item.product_sku_id AND location_id = v_src_item.source_location_id AND quantity > 0 AND status = 'AVAILABLE' ORDER BY received_date ASC, created_at ASC FOR UPDATE LOOP
    ...
    END LOOP;
    UPDATE wh_repacking_orders SET status = 'COMPLETED', updated_at = NOW() WHERE id = p_order_id;
    RETURN p_order_id;
END;
$$ LANGUAGE plpgsql;