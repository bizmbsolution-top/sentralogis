...
BEGIN
    ...
    FOR v_src_item IN SELECT * FROM wh_repacking_items WHERE repacking_order_id = p_order_id AND item_type = 'SOURCE' LOOP
        ...
        FOR v_source_inv IN SELECT * FROM wh_inventory WHERE tenant_id = v_order.tenant_id AND warehouse_id = v_order.warehouse_id AND product_sku_id = v_src_item.product_sku_id AND location_id = v_src_item.source_location_id AND quantity > 0 ORDER BY received_date ASC, created_at ASC FOR UPDATE LOOP
            ...
            INSERT INTO wh_inventory_movements (tenant_id, inventory_id, movement_type, from_location_id, quantity, reference_type, reference_id, notes, created_by) VALUES (v_order.tenant_id, v_source_inv.id, 'KITTING_CONSUME', v_src_item.source_location_id, v_deduct, v_order.order_type, v_order.id, 'Consumed for ' || v_order.order_type || ' order: ' || v_order.order_number, p_user_id);
    ...
    FOR v_res_item IN SELECT * FROM wh_repacking_items WHERE repacking_order_id = p_order_id AND item_type = 'RESULT' LOOP
        ...
        IF v_dest_inv_id IS NULL THEN
            INSERT INTO wh_inventory (tenant_id, warehouse_id, customer_id, product_sku_id, location_id, quantity, status, batch_number, expiry_date, unit_cost, created_by) VALUES (v_order.tenant_id, v_order.warehouse_id, v_order.customer_id, v_res_item.product_sku_id, v_res_item.target_location_id, v_res_item.quantity, 'AVAILABLE', v_res_item.batch_number, v_res_item.expiry_date, v_res_item.unit_cost, p_user_id) RETURNING id INTO v_dest_inv_id;
        ELSE
            UPDATE wh_inventory SET quantity = quantity + v_res_item.quantity, updated_at = NOW() WHERE id = v_dest_inv_id;
        END IF;
        INSERT INTO wh_inventory_movements (tenant_id, inventory_id, movement_type, to_location_id, quantity, reference_type, reference_id, notes, created_by) VALUES (v_order.tenant_id, v_dest_inv_id, 'KITTING_OUTPUT', v_res_item.target_location_id, v_res_item.quantity, v_order.order_type, v_order.id, 'Result from ' || v_order.order_type || ' order: ' || v_order.order_number, p_user_id);
    END LOOP;
    v_jo_number := 'JO-' || to_char(NOW(), 'YYMMDD') || '-' || SUBSTRING(gen_random_uuid()::text, 1, 4);
    INSERT INTO wo_items (tenant_id, sbu_type, status, item_data, created_by) VALUES (v_order.tenant_id, 'WAREHOUSE', 'completed', jsonb_build_object('order_id', p_order_id, 'warehouse_id', v_order.warehouse_id, 'customer_id', v_order.customer_id, 'operation_type', v_order.order_type, 'direction', 'REPACKING'), p_user_id) RETURNING id INTO v_wo_item_id;
    INSERT INTO job_orders (tenant_id, wo_item_id, jo_number, status, sbu_type, customer_id, description) VALUES (v_order.tenant_id, v_wo_item_id, v_jo_number, 'completed', 'WAREHOUSE', v_order.customer_id, v_order.order_type || ' Order: ' || v_order.order_number);
