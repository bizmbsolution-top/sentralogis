-- ============================================================================
-- SENTRALOGIS DATA-4E-R8 FINAL DETERMINISTIC REPAIR
-- STATUS: NOT AUTHORIZED FOR EXECUTION
-- DO NOT DEPLOY
-- Architecture: ADR-075 fw_locations Migration
-- ============================================================================
-- CRITICAL FIX vs MIGRATION 044:
-- Migration 044 tried to transform FK values BEFORE dropping old FK constraints.
-- This fails because the old constraint requires values to exist in fw_locations,
-- but we're trying to set them to md_locations.id values.
--
-- Correct order:
-- 1. Create md_locations
-- 2. DROP old FK constraints (so we can freely update values)
-- 3. TRANSFORM FK values (UPDATE to md_locations.id)
-- 4. ADD new FK constraints (validates values exist in md_locations)
--
-- R8 CLOSURE ITEMS:
-- R8-01: Explicit FK value state machine
-- R8-02: Canonical record resolution
-- R8-03: Migration-created canonical provenance (transaction-only rollback)
-- R8-04: FK constraint state machine
-- R8-05: ON DELETE/ON UPDATE equivalence
-- ============================================================================
...
-- ============================================================================
-- PHASE 2: CANONICAL RECORD RESOLUTION
-- ============================================================================
...
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM public.fw_locations fl
    JOIN public.md_locations ml ON ml.tenant_id = fl.tenant_id AND ml.external_code = fl.location_id::TEXT
    WHERE ml.name != fl.name OR ml.location_type != fl.type) THEN
    RAISE EXCEPTION 'Semantic equivalence violation';
  END IF;
END $$;

-- ============================================================================
-- PHASE 3: DROP OLD FK CONSTRAINTS (BEFORE value transformation)
-- ============================================================================
...
-- Only transform values that are in LEGACY state (reference fw_locations)
...
-- ============================================================================
-- PHASE 5: ADD NEW FK CONSTRAINTS (idempotent: drop if exists first)
-- ============================================================================
...
BEGIN
  ...
  -- Tenant checks (4/4)
  IF EXISTS (SELECT 1 FROM public.fw_order_headers oh JOIN public.md_locations ml ON ml.id = oh.origin_port_id WHERE oh.tenant_id != ml.tenant_id) THEN RAISE EXCEPTION 'origin_port_id tenant violation'; END IF;
  IF EXISTS (SELECT 1 FROM public.fw_order_headers oh JOIN public.md_locations ml ON ml.id = oh.dest_port_id WHERE oh.tenant_id != ml.tenant_id) THEN RAISE EXCEPTION 'dest_port_id tenant violation'; END IF;
  IF EXISTS (SELECT 1 FROM public.fw_legs leg JOIN public.md_locations ml ON ml.id = leg.start_location_id WHERE leg.tenant_id != ml.tenant_id) THEN RAISE EXCEPTION 'start_location_id tenant violation'; END IF;
  IF EXISTS (SELECT 1 FROM public.fw_legs leg JOIN public.md_locations ml ON ml.id = leg.end_location_id WHERE leg.tenant_id != ml.tenant_id) THEN RAISE EXCEPTION 'end_location_id tenant violation'; END IF;
