-- Migration 142: Execute Repacking Order RPC

-- Add workflow columns
ALTER TABLE wh_repacking_orders ADD COLUMN IF NOT EXISTS executed_by UUID;
ALTER TABLE wh_repacking_orders ADD COLUMN IF NOT EXISTS executed_at TIMESTAMPTZ;
ALTER TABLE wh_repacking_orders ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;
ALTER TABLE wh_repacking_orders ADD COLUMN IF NOT EXISTS reference_id TEXT;
...
BEGIN
    ...
    LOOP
        ...
        -- We loop through inventory to deduct (FIFO)
        FOR v_source_inv IN 
        ...
        IF v_qty_remaining > 0 THEN
            RAISE EXCEPTION 'Insufficient stock in source location % for SKU %', v_src_item.source_location_id, v_src_item.product_sku_id;
        END IF;
    END LOOP;
    ...
    LOOP
        ...
        ELSE
            INSERT INTO wh_inventory (
                ...
                v_order.tenant_id, v_order.warehouse_id, v_res_item.target_location_id, v_res_item.product_sku_id, v_res_item.quantity,
