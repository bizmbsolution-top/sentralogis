...
export class RabbitMQDomainEventDispatcher implements IDomainEventDispatcher {
  async dispatchSynchronous<TEvent extends DomainEvent>(envelope: EventEnvelope<TEvent>): Promise<DispatchResult> {
    ...
    return { success: true, dispatcherName: 'RabbitMQDomainEventDispatcher', latencyMs: 0, retryCount: 0 };
  }
