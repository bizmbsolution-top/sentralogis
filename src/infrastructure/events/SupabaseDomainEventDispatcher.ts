...
export class SupabaseDomainEventDispatcher implements IDomainEventDispatcher {
  async dispatchSynchronous<TEvent extends DomainEvent>(envelope: EventEnvelope<TEvent>): Promise<DispatchResult> {
    ...
    return { success: true, dispatcherName: 'SupabaseDomainEventDispatcher', latencyMs: 0, retryCount: 0 };
  }
