...
export class EventBridgeDomainEventDispatcher implements IDomainEventDispatcher {
  async dispatchSynchronous<TEvent extends DomainEvent>(envelope: EventEnvelope<TEvent>): Promise<DispatchResult> {
    ...
    return { success: true, dispatcherName: 'EventBridgeDomainEventDispatcher', latencyMs: 0, retryCount: 0 };
  }
