...
export class MemoryDomainEventDispatcher implements IDomainEventDispatcher {
  async dispatchSynchronous<TEvent extends DomainEvent>(envelope: EventEnvelope<TEvent>): Promise<DispatchResult> {
    ...
    return { success: true, dispatcherName: 'MemoryDomainEventDispatcher', latencyMs: 0, retryCount: 0 };
  }
