...
try {
    ...
    // Marker for the end of the Work Order list
    const markers = [
        '</span>',
        '</div>',
        '</div>',
    ...
    // Let's just do a direct surgical replacement of the broken section
    // Finding from the end of the items map to the mobile nav
    
    const startOfProblem = 'Rp {item.deal_price?.toLocaleString(\'id-ID\')}</span>';
    const endOfProblem = '<span className="text-[8px] font-black uppercase tracking-widest">Customer</span>';
    
    if (content.includes(startOfProblem) && content.includes(endOfProblem)) {
        const parts = content.split(startOfProblem);
        const secondHalf = parts[1].split(endOfProblem);
        ...
        content = parts[0] + startOfProblem + fixedSection + endOfProblem + secondHalf[1];
        fs.writeFileSync(path, content);
        console.log("SBU Fix: Restoration complete and verified.");
    } else {
