...
async function checkCSPolicy() {
  const { data, error } = await supabase.rpc('exec_sql_manual', { 
    sql_query: "SELECT qual FROM pg_policies WHERE tablename = 'job_routes' AND policyname = 'cs_full_access_job_routes'" 
  });
  
  if (error) {
     console.error(error);
  } else {
     console.log('CS Policy Qual:', data[0].qual);
  }
}
