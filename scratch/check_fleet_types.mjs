...
async function checkFleetTypeColumns() {
  const { data, error } = await supabase.from('md_fleet_types').select('*').limit(1)
  console.log('md_fleet_types columns:', Object.keys(data[0] || {}))
}

checkFleetTypeColumns()
