...
async function checkAttachments() {
  const { data, error } = await supabase.from('job_orders').select('pod_photo_url, advance_receipt_url, transfer_proof_url').limit(1)
  console.log('job_orders sample docs:', data)
}
