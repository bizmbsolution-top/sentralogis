...
import {
  Loader2, ArrowDownUp, Search, Filter, Plus,
  Package, AlertCircle, CheckCircle2, Clock, Scan
} from "lucide-react";
...
export default function SBUInboundPage() {
  ...
  async function fetchTasks() {
    try {
      ...
      console.error('Failed to fetch inbound tasks:', e);
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-slate-900">Inbound</h1>
          <p className="text-slate-500 text-sm mt-1">Receiving & Putaway Tasks</p>
        </div>
        <button className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-xl hover:bg-orange-600 transition-colors text-sm font-medium">
          <Plus size={16} />
          New Receipt
        </button>
      </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 && (
          <Card className="p-12 text-center">
            ...
            <p className="text-slate-500 text-sm">No inbound tasks found</p>
          </Card>
        )}
        {filtered.map((task) => (
