...
export default function FleetHubPage() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (!tenantId) {
          console.warn("Tenant ID not found in profile. Data may be filtered differently.");
      }
  ...
  const resetForm = () => {
    setFormData(activeTab === 'fleets' ? {
        plate_number: "", fleet_type_id: "", brand: "", color: "", year: "",
        status: "available", entity_id: "", stnk_expiry: "", kir_expiry: "", notes: "", tenant_id: userOrgId
    } : activeTab === 'drivers' ? {
        name: "", phone: "", license_type: "BII Umum", license_number: "", license_expiry: "", nik: "",
        status: "available", entity_id: "", address: "", notes: "", tenant_id: userOrgId,
        bank_name: "", bank_account_number: "", bank_account_name: ""
    } : {
        name: "", type: "vendor", address: "", pic_name: "", pic_phone: "", notes: "", is_active: true, tenant_id: userOrgId
    });
    setEditingId(null);
  };
  ...
  const handleSave = async () => {
    try {
      const table = activeTab === 'fleets' ? 'md_fleets' : activeTab === 'drivers' ? 'md_drivers' : 'md_entities';
      
      // Clean payload from foreign relational objects and metadata
      const payload = { ...formData };
      delete payload.md_entities;
      delete payload.md_fleet_types;
      delete payload.created_at;
      delete payload.updated_at;
      
      // Convert empty strings to null for UUIDs and optional dates/numbers
      if (payload.entity_id === "") payload.entity_id = null;
      if (payload.fleet_type_id === "") payload.fleet_type_id = null;
      if (payload.stnk_expiry === "") payload.stnk_expiry = null;
      if (payload.kir_expiry === "") payload.kir_expiry = null;
      if (payload.license_expiry === "") payload.license_expiry = null;
      if (payload.year === "" || payload.year === 0) payload.year = null;
      if (payload.tenant_id === "") payload.tenant_id = null;

      if (editingId) {
        const idToUpdate = editingId;
        delete payload.id; // don't push PK on update
        const { error } = await supabase.from(table).update(payload).eq('id', idToUpdate);
        if (error) throw error;
        toast.success("Record Identity Updated!");
      } else {
        delete payload.id; // ensure no ID on insert
  ...
  const filteredData = activeTab === 'fleets' 
    ? fleets.filter(f => `${f.plate_number} ${f.md_fleet_types?.type_name}`.toLowerCase().includes(searchTerm.toLowerCase()))
    : activeTab === 'drivers' 
  ...
  return (
    <div className="min-h-screen bg-[#F1F5F9] text-[#1E293B] pb-40">
      ...
      <header className="sticky top-0 z-[100] bg-white border-b border-slate-200 px-4 py-6 flex flex-col md:flex-row justify-between items-center shadow-sm gap-6">
      ...
      <main className="w-full py-8 lg:py-12 space-y-12">
                    ...
                    <div className="relative z-10 flex-1">
                       ...
                       <div className="space-y-3 mb-8">
                          <div className="flex items-center gap-3">
                             ...
                             {activeTab === 'fleets' ? (item.md_fleet_types?.type_name || 'Unknown Type') : activeTab === 'drivers' ? (item.license_type || 'Pilot') : (item.type === 'company' ? 'Internal Asset Group' : 'External Vendor')}
                             </span>
                          ...
                          {activeTab === 'companies' ? (
                             ...
                             <>
                                <div className="flex items-center gap-4 text-slate-500">
                                   ...
                                   <span className="text-[11px] font-black uppercase italic tracking-tight">{item.brand || 'Standard Engine'}</span>
                                </div>
                                <div className="flex items-center gap-4 text-slate-500">
                                   ...
                                   <span className="text-[11px] font-black uppercase italic tracking-tight line-clamp-1">{item.md_entities?.name || 'Local Asset'}</span>
                                </div>
                             </>
                          ) : (
                             <>
                 ...
                 {activeTab === 'companies' ? (
                   ...
                   <>
                      ...
                      <div className="space-y-3">
                         ...
                         <select className="w-full h-18 px-8 bg-slate-50 border border-slate-200 rounded-[1.8rem] text-sm font-black text-[#1E293B] outline-none appearance-none" value={formData.fleet_type_id || ''} onChange={(e) => setFormData({ ...formData, fleet_type_id: e.target.value })}>
                            <option value="">Select Type</option>
                            {truckTypes.map(t => <option key={t.id} value={t.id}>{t.type_name}</option>)}
                         </select>
                      </div>
                      <div className="space-y-3">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1">Truck Brand</label>
                         <input type="text" className="w-full h-18 px-8 bg-slate-50 border border-slate-200 rounded-[1.8rem] text-sm font-black text-[#1E293B] outline-none focus:bg-white focus:border-[#1E293B] transition-all uppercase" placeholder="Hino / Fuso" value={formData.brand || ''} onChange={(e) => setFormData({ ...formData, brand: e.target.value })} />
                      </div>
                      <div className="space-y-3">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1">Truck Color</label>
                         <input type="text" className="w-full h-18 px-8 bg-slate-50 border border-slate-200 rounded-[1.8rem] text-sm font-black text-[#1E293B] outline-none focus:bg-white focus:border-[#1E293B] transition-all" placeholder="Green" value={formData.color || ''} onChange={(e) => setFormData({ ...formData, color: e.target.value })} />
                      </div>
                      <div className="space-y-3">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1">Year Manufacture</label>
                         <input type="number" className="w-full h-18 px-8 bg-slate-50 border border-slate-200 rounded-[1.8rem] text-sm font-black text-[#1E293B] outline-none focus:bg-white focus:border-[#1E293B] transition-all" placeholder="20XX" value={formData.year || ''} onChange={(e) => setFormData({ ...formData, year: e.target.value })} />
                      </div>
                      <div className="space-y-3">
                      ...
                      <div className="space-y-3">
                         ...
                         <input type="date" className="w-full h-18 px-8 bg-slate-50 border border-slate-200 rounded-[1.8rem] text-sm font-black text-[#1E293B] outline-none focus:bg-white focus:border-[#1E293B] transition-all" value={formData.kir_expiry || ''} onChange={(e) => setFormData({ ...formData, kir_expiry: e.target.value })} />
                      </div>
                      <div className="space-y-3 md:col-span-2">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1">Notes</label>
                   ...
                   <>
                      ...
                      <div className="space-y-3 md:col-span-2">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block px-1 text-blue-600">🏦 PILOT BANK ACCOUNT</label>
                         <div className="grid grid-cols-1 md:grid-cols-3 gap-4 bg-blue-50 p-6 rounded-[1.8rem] border border-blue-100">
                            <div className="space-y-2">
                               <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Bank Name</label>
                               <input type="text" className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-xs font-black text-[#1E293B] outline-none" placeholder="BCA / Mandiri" value={formData.bank_name || ''} onChange={(e) => setFormData({ ...formData, bank_name: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                               <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Account Number</label>
                               <input type="text" className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-xs font-black text-[#1E293B] outline-none" placeholder="XXXX-XXXX" value={formData.bank_account_number || ''} onChange={(e) => setFormData({ ...formData, bank_account_number: e.target.value })} />
                            </div>
                            <div className="space-y-2">
                               <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest block">Account Holder</label>
                               <input type="text" className="w-full h-12 px-4 bg-white border border-slate-200 rounded-xl text-xs font-black text-[#1E293B] outline-none" placeholder="Name on Card" value={formData.bank_account_name || ''} onChange={(e) => setFormData({ ...formData, bank_account_name: e.target.value })} />
                            </div>
                         </div>
                      </div>
