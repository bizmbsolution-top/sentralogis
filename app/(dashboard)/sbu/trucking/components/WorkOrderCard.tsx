                            ...
                            <div className="space-y-3">
                                ...
                                <div className="space-y-2">
                                    {item.assignments.slice(0, 5).map((a, idx) => (
                                            ...
                                            >
                                                <div 
                                                    className={`flex items-center gap-3 min-w-0 flex-1 ${woStatus === 'handover_pending' ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`} 
                                                    onClick={() => woStatus !== 'handover_pending' && onOpenDetails(a)}
                                                >
                                                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors border ${a.is_link_sent ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-slate-50 text-slate-400 border-slate-100'} ${woStatus !== 'handover_pending' && 'group-hover/jo:scale-105'}`}>
                                                ...
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <ChevronRight 
                                                        onClick={() => woStatus !== 'handover_pending' && onOpenDetails(a)} 
                                                        className={`w-4 h-4 transition-transform ${woStatus === 'handover_pending' ? 'text-slate-200 cursor-not-allowed' : 'text-slate-300 group-hover/jo:text-emerald-600 group-hover/jo:translate-x-1 cursor-pointer'}`} 
                                                    />
                                                </div>
                                            </div>
                                    ))}
                ...
                <div className="pt-4">
                    {(woStatus === 'draft' || woStatus === 'pending_sbu') ? (
                        ...
                        <div className="flex flex-col gap-2.5">
                            <button 
                                onClick={() => {
                                    if (woStatus === 'handover_pending') return;
                            ...
                            >
                                <Banknote className={`w-4.5 h-4.5 ${woStatus === 'handover_pending' ? 'text-slate-300' : 'text-emerald-400 group-hover:scale-110'} transition-transform`} /> 
                                {woStatus === 'handover_pending' ? 'Locked (Pending Approval)' : 'Cost Settlement & Logs'}
                            </button>
                            <div className="grid grid-cols-2 gap-3">
                                <button 
