...
import { Download, Package, Clock, CheckCircle2 } from "lucide-react";
...
export default function HQWarehouseInbound() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const { data } = await supabase
        .from("wh_tasks")
        .select(`
          id, task_number, task_type, status, priority,
  ...
  const statCards = [
    ...
    { label: "In Progress", value: tasks.filter(t => t.status === "IN_PROGRESS").length, icon: Package, color: "text-amber-600", bg: "bg-amber-50" },
    { label: "Completed", value: tasks.filter(t => t.status === "COMPLETED").length, icon: CheckCircle2, color: "text-emerald-600", bg: "bg-emerald-50" },
    { label: "Total", value: tasks.length, icon: Download, color: "text-slate-600", bg: "bg-slate-50" },
  ];

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-900">Inbound Management</h1>
        <p className="text-slate-500 text-sm mt-1">Monitor penerimaan barang dan putaway</p>
      </div>
      ...
      {loading ? (
        ...
        <Card>
          <CardHeader>
            <h2 className="text-sm font-bold text-slate-900">Inbound Tasks</h2>
          </CardHeader>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-50">
                {tasks.length === 0 ? (
                  <tr><td colSpan={7} className="text-center py-12 text-slate-400">No inbound tasks yet</td></tr>
                ) : tasks.map((t) => (
                  <tr key={t.id} className="hover:bg-slate-50/50">
                    <td className="px-4 py-3 font-mono text-xs font-medium">{t.task_number}</td>
                    <td className="px-4 py-3"><Badge variant="info">{t.task_type}</Badge></td>
