...
export default function CreateWOForm({ onBack, editId }: CreateWOFormProps) {
  ...
  useEffect(() => {
    const fetchCustomers = async () => {
      // Tunggu sampai profile dan tenant_id benar-benar ada
      if (!profile?.tenant_id) {
        console.log('Waiting for stable profile...');
        return;
      }
      
      try {
        console.log('Executing fetch for tenant:', profile.tenant_id);
        const { data, error } = await supabase
          .from('md_entities')
          .select('id, name, legal_name, is_customer, is_vendor')
          .eq('tenant_id', profile.tenant_id)
          .or('is_customer.eq.true,is_vendor.eq.true')
          .order('name', { ascending: true });
        
        if (error) throw error;
        
        if (data) {
          console.log(`Successfully synced ${data.length} customers.`);
          setCustomers(data);
        }
      } catch (err) {
    ...
    fetchCustomers();
  }, [profile?.tenant_id]); // Hanya trigger jika tenant_id berubah
  ...
  const handleSubmit = async (status: 'draft' | 'need_assignment') => {
    ...
    try {
      console.log('1. Starting Submit Process...');
      let woNumber = '';
      let woId = editId;

      if (editId) {
        // Use existing WO number for editing
        const { data: existingWO } = await supabase.from('work_orders').select('wo_number').eq('id', editId).single();
        woNumber = existingWO?.wo_number || '';
      } else {
        woNumber = await generateWONumber(profile.tenant_id);
      }
      
      console.log('2. WO Number:', woNumber);
      ...
      const payload = {
        ...
        status,
        created_by: profile?.id || profile?.user_id
      };

      let woError;
      if (editId) {
        console.log('3. Updating WO Header:', editId);
        const { error } = await supabase.from('work_orders').update(payload).eq('id', editId);
        woError = error;
      } else {
        console.log('3. Inserting WO Header');
      ...
      if (woError) {
        console.error('4. WO Header Error:', woError);
        throw woError;
      }
      console.log('5. WO Header Success:', woId);

      // If editing, clear old items first
      if (editId) {
        console.log('5b. Clearing old items for refresh');
        await supabase.from('wo_items').delete().eq('wo_id', editId);
      }

      for (const [index, item] of woItems.entries()) {
        const itemCode = `${woNumber}-ITM-${(index + 1).toString().padStart(2, '0')}`;
        console.log(`6.${index} Inserting Item:`, itemCode);
        ...
        if (itemError) {
          console.error(`6.${index} Item Error:`, itemError);
          throw itemError;
        }

        // FASE 1: MULTI-STOP ROUTING (Pre-create Job Orders and Routes)
        if (item.sbu_type === 'TRUCKING' && item.item_data.stops) {
          const unitCount = item.item_data.unit_count || 1;
          for (let i = 1; i <= unitCount; i++) {
            const joNumber = `${itemCode}-JO-${i.toString().padStart(3, '0')}`;
            ...
            // Insert Routes for this Job Order
            const routePayloads = item.item_data.stops.map((stop: any) => ({
              job_order_id: jobOrder.id,
              sequence: stop.sequence,
              stop_type: stop.stop_type,
              source_type: stop.source_type === 'GOOGLE_MAPS' ? 'MD_CONTACT_ADDRESS' : 'MD_LOCATION',
      ...
      toast.success(editId ? 'Perubahan berhasil disimpan' : (status === 'draft' ? 'Draft berhasil disimpan' : 'WO berhasil dikirim ke SBU'));
      onBack();
    } catch (err: any) {
      console.error('SUBMIT FATAL ERROR:', err);
      toast.error(err.message || 'Gagal menyimpan Work Order.');
    } finally {
      console.log('8. Submit Process Finished (Finally)');
      setSubmitting(null);
    }
  };
  ...
  return (
    <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-4">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-start gap-4">
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className="text-xs font-black text-slate-900 uppercase tracking-wider">{item.item_data.unit_count} Units Deployment — {item.item_data.vehicle_type_name}</span>
                        </div>
                        ...
                        <div className="flex items-center gap-1.5 text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">
                           <MapPin size={12} className="text-rose-500" /> {item.item_data.shipper_address}
                        </div>
                    ...
                    <div className="flex items-center gap-4 pr-4 border-l border-slate-100 pl-6">
                       <div className="text-right min-w-[120px]">
                          ...
                          <p className="text-sm font-black text-slate-900 italic font-mono">IDR {item.item_data.est_revenue.toLocaleString('id-ID')}</p>
                       </div>
