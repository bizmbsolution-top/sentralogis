...
import { getQuotationPublic } from '@/app/quote/actions';
...
export default function QuotationPrintPreview({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSendViaWhatsApp = () => {
    const custName = entity?.name || 'Bapak/Ibu';
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col items-center">
      ...
      <div className="fixed bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3 bg-white px-4 py-3 rounded-full shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-slate-200 z-50 print:hidden">
        ...
        <button onClick={handleSendViaWhatsApp} className="px-5 h-10 bg-emerald-600 hover:bg-emerald-700 text-white rounded-full font-bold text-sm flex items-center gap-2 shadow-sm transition-colors">
          ...
          <div className="flex justify-between mb-8">
            ...
            <div className="w-1/3 text-right">
              <div className="mb-3">
                ...
                <p className="text-xs font-bold text-slate-800">{quote.created_at ? new Date(quote.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }) : '-'}</p>
              </div>
              <div className="mb-3">
                          ...
                          {item.sbu_cluster === 'TRUCKING' && item.sbu_metadata && (
                            <div className="mt-1.5 p-2 bg-slate-50 border border-slate-100 rounded text-[10px] text-slate-600 space-y-1 font-normal">
                              ...
                              {item.sbu_metadata.stops && item.sbu_metadata.stops.length > 0 && (
                                <div className="pl-1 border-l border-slate-350">
