  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex flex-col items-center justify-center p-8">
      ...
      <div className="bg-white border border-slate-200 rounded-3xl p-12 max-w-2xl text-center shadow-sm">
        <div className="mx-auto w-20 h-20 rounded-full bg-violet-50 flex items-center justify-center mb-6">
          <FileText className="w-10 h-10 text-violet-600" />
        </div>
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Billing & Invoicing</h1>
        <p className="text-slate-500 mb-8">Status invoice, unbilled JO, billing cycle time, invoice aging.</p>
        <div className="bg-violet-50 border border-violet-200 rounded-xl p-4 text-left">
          <p className="text-sm font-semibold text-violet-800 mb-2">🚧 Coming Soon (Phase 3)</p>
          <p className="text-xs text-violet-700">Fitur akan mencakup:</p>
          <ul className="text-xs text-violet-700 list-disc list-inside space-y-1 mt-2">
            <li>Invoice status: draft, sent, paid, overdue</li>
            <li>Unbilled completed JOs</li>
            <li>Billing cycle time (JO complete → invoice sent)</li>
            <li>Invoice aging & follow-up actions</li>
          </ul>
        </div>
      </div>
    </div>
