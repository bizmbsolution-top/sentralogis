...
import { Search, Wallet, ChevronDown, Coins, LogOut, Building2 } from "lucide-react";
import Link from "next/link";
import Image from "next/image";

interface ForwardingHeaderProps {
            ...
            <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-8">
                ...
                <div className="flex items-center gap-4 flex-shrink-0">
                    ...
                    <div className="hidden sm:block">
                        ...
                        <h1 className="text-sm font-black text-slate-900 uppercase tracking-tight italic">{tenantInfo.name || 'FORWARDING'}</h1>
                    </div>
                </div>
                ...
                <div className="flex-1 max-w-2xl relative group">
                    ...
                    <input 
                        type="text" 
                        placeholder="SEARCH CONSOLIDATION, BOOKING, OR SHIPMENT..." 
                        value={searchTerm}
                        onChange={(e) => setSearchTerm && setSearchTerm(e.target.value)}
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-12 pr-6 py-3 text-[11px] font-bold text-slate-900 focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 transition-all outline-none placeholder:text-slate-400 uppercase tracking-widest" 
                    />
                </div>

                {/* 3. PROFILE & ACTIONS */}
                <div className="flex items-center gap-4 flex-shrink-0">
                    <Link href="/sbu/forwarding/finances" className="h-11 px-4 rounded-xl bg-slate-100 border border-slate-200 flex items-center gap-3 hover:bg-white hover:shadow-xl transition-all group/finance">
                    ...
                    <div className="relative">
                        ...
                        >
                            ...
                            <div className="text-left hidden lg:block">
                                <p className="text-[10px] font-black text-slate-900 leading-none mb-1 uppercase tracking-tight">{userProfile?.full_name || 'FORWARDING TEAM'}</p>
                                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">Global Agent</p>
                            </div>
                            ...
                            <>
                                ...
                                <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 shadow-2xl rounded-[2rem] p-4 z-50 animate-in zoom-in-95 duration-200 origin-top-right">
                                    <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-2">
                                        <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Fuel & Ops Credits</p>
