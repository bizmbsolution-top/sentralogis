...
import { 
  ...
  ChevronDown as ChevronDownIcon, Download as DownloadIcon, Printer as PrinterIcon, X as XIcon, LayoutGrid as LayoutGridIcon, FileText as FileTextIcon, History as HistoryIcon,
  ...
  Gavel as GavelIcon, Scale as ScaleIcon, LogOut
} from 'lucide-react';
...
// Standard Components
import ClearanceHeader from "../components/ClearanceHeader";
import ClearanceHero from "../components/ClearanceHero";

export default function ClearancesFinanceDashboard() {
    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <main className="max-w-[1600px] mx-auto px-8 py-10 space-y-10">
        ...
        <ClearanceHero 
            title={(
                <>
                    Regulatory Precision,<br/>
                    <span className="text-blue-600">Compliance Integrity</span>
                </>
            )}
        />
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
          {[
            { label: 'Clearance Revenue', val: `Rp ${stats.totalRevenue.toLocaleString()}`, sub: 'Unbilled Entries', icon: TrendingUpIcon, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
            { label: 'SBU Petty Cash', val: `Rp ${stats.pettyCashBalance.toLocaleString()}`, sub: 'Duty & Tax Float', icon: WalletIcon, color: 'text-indigo-600', bg: 'bg-indigo-50', border: 'border-indigo-100' },
            { label: 'Active Filings', val: stats.activeMissions, sub: 'Customs Gateways', icon: FileTextIcon, color: 'text-emerald-600', bg: 'bg-emerald-50', border: 'border-emerald-100' },
            { label: 'Auditor Matrix', val: 'Synchronized', sub: 'Matrix Secure', icon: ShieldCheckIcon, color: 'text-rose-600', bg: 'bg-rose-50', border: 'border-rose-100' },
          ].map((card, i) => (
        ...
        {activeTab === 'billing' && (
          <div className="bg-white rounded-[3rem] border border-slate-200 shadow-sm overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-700">
             <div className="p-10 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
                <div>
                   <h3 className="text-xl font-black italic tracking-tighter text-slate-900 uppercase">Clearance Fiscal Matrix</h3>
                   <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest mt-1">Audit active customs filing missions</p>
                </div>
                <div className="flex items-center gap-4">
                   <div className="relative">
                      ...
                      <input 
                        type="text" 
                        placeholder="SEARCH CUS# OR CUSTOMER..." 
                        className="pl-12 pr-6 py-4 bg-white border border-slate-200 rounded-2xl text-[10px] font-black uppercase tracking-widest focus:outline-none focus:border-blue-500 transition-all w-72 shadow-sm"
                      />
                   </div>
                </div>
             </div>
                ...
                <table className="w-full text-left border-separate border-spacing-y-4">
                   <thead>
                      <tr className="text-[9px] font-black text-slate-400 uppercase tracking-[0.3em] italic">
                         <th className="pb-4 px-8">Filing ID</th>
                         <th className="pb-4">Declarant Entity</th>
                         <th className="pb-4">Gateway Type</th>
                        ...
                        <tr>
                          <td colSpan={5} className="py-24 text-center">
                            <div className="flex flex-col items-center gap-4 text-slate-300">
                               ...
                               <p className="text-xs font-black uppercase tracking-widest italic opacity-40">No Regulatory Missions Logged</p>
                            </div>
                          </td>
                        </tr>
                        ...
                        <tr key={idx} className="group hover:bg-slate-50 transition-all">
                           <td className="bg-slate-50/30 rounded-l-[2rem] p-8 border-y border-l border-slate-100">
                              <div className="flex items-center gap-5">
                                 <div className="w-14 h-14 bg-white rounded-2xl flex items-center justify-center border border-slate-200 text-blue-600 shadow-sm group-hover:scale-110 transition-transform">
                                    <GavelIcon className="w-6 h-6" />
                                 </div>
                                 <div className="leading-tight">
                                    <p className="text-[13px] font-black italic text-slate-900 uppercase tracking-tight">CUS-{jo.jo_number?.split('-').pop()}</p>
                           ...
                           <td className="bg-slate-50/30 p-8 border-y border-slate-100">
                              ...
                              <div className="flex items-center gap-2 mt-2">
                                 ...
                                 <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Regulatory Partner</span>
                              </div>
                           </td>
                           <td className="bg-slate-50/30 p-8 border-y border-slate-100">
                              <p className="text-[11px] font-black text-slate-900 uppercase italic">Customs Export/Import</p>
                              <p className="text-[9px] font-bold uppercase tracking-widest mt-1.5 text-slate-400">Tax Matrix Protocol</p>
                           </td>
                           <td className="bg-slate-50/30 p-8 border-y border-slate-100">
                              <div className={`px-4 py-2 rounded-xl border w-fit flex items-center gap-2.5 ${jo.billing_status === 'paid' ? 'bg-emerald-50 border-emerald-100 text-emerald-600 shadow-sm shadow-emerald-500/5' : 'bg-orange-50 border-orange-100 text-orange-600'}`}>
                                 ...
                                 <span className="text-[9px] font-black uppercase tracking-widest italic">{jo.billing_status?.toUpperCase() || 'DOCUMENTED'}</span>
                              </div>
                           </td>
                           <td className="bg-slate-50/30 rounded-r-[2rem] p-8 border-y border-r border-slate-100 text-right">
                              <button className="px-6 py-3 bg-slate-900 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all active:scale-95 shadow-xl shadow-slate-900/10 italic">
                                 Audit Unit
                              </button>
                           </td>
                        </tr>
                   ...
                   <div className="space-y-10">
                      <div className="space-y-4">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2">
                             <div className="w-1 h-3 bg-slate-200" /> Description / Service
                         </label>
                         <input type="text" placeholder="DUTY FEES / TAXES / ETC" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-6 px-8 text-xs font-black italic text-slate-900 focus:outline-none focus:border-blue-500/30 transition-all font-mono shadow-inner outline-none" />
                      </div>
                      <div className="space-y-4">
                         <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest italic flex items-center gap-2">
                            <div className="w-1 h-3 bg-slate-200" /> Quantum Amount (Rp)
                         </label>
                         <input type="number" placeholder="0" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-6 px-8 text-2xl font-black italic tracking-tighter text-slate-900 focus:outline-none focus:border-blue-500/30 transition-all shadow-inner outline-none" />
                      </div>
                ...
                <div className="bg-[#111214] rounded-[3.5rem] border border-white/5 p-12 shadow-2xl flex flex-col min-h-[700px] relative overflow-hidden">
                   ...
                   <div className="flex justify-between items-center mb-12 relative z-10">
                      <div className="flex items-center gap-4">
                        ...
                        <h3 className="text-[11px] font-black text-white/40 tracking-[0.4em] uppercase italic">Regulatory Sync Log</h3>
                      </div>
                      <button className="text-[10px] font-black text-blue-400 uppercase tracking-widest flex items-center gap-3 hover:text-blue-300 transition-colors bg-white/5 px-6 py-2.5 rounded-full border border-white/5">
                      ...
                      {pettyCash.length === 0 ? (
                        <div className="py-32 text-center flex flex-col items-center gap-8">
                           ...
                           <div className="space-y-3">
                              ...
                              <p className="text-[10px] font-bold text-white/20 uppercase tracking-[0.3em]">No regulatory payments detected in current session</p>
                           </div>
                        </div>
                      ) : pettyCash.map((t, idx) => (
