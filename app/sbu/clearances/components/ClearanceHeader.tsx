    ...
    <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200 sticky top-0 z-[100] px-6 py-4">
      <div className="max-w-[1600px] mx-auto flex items-center justify-between gap-8">
        ...
        <div className="flex items-center gap-4 flex-shrink-0">
          <Link href="/sbu-launchpad" className="w-10 h-10 bg-slate-900 rounded-xl flex items-center justify-center shadow-lg shadow-slate-900/10 overflow-hidden hover:scale-105 transition-all active:scale-95">
            <ArrowLeft className="w-5 h-5 text-white" />
          </Link>
          <div className="hidden sm:block">
            <p className="text-[10px] font-black text-slate-400 tracking-[0.2em] uppercase leading-none mb-1">Clearance Division</p>
            <h1 className="text-sm font-black text-slate-900 uppercase tracking-tight italic">{tenantInfo.name || 'CLEARANCES'}</h1>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-2xl relative group">
          ...
          <input 
            type="text" 
            placeholder="SEARCH BY WO#, CUSTOMER, OR AJU NUMBER..." 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-slate-50 border border-slate-200 rounded-2xl pl-12 pr-6 py-3 text-[11px] font-bold text-slate-900 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all outline-none placeholder:text-slate-400 uppercase tracking-widest" 
          />
        </div>

        {/* 3. PROFILE & ACTIONS */}
        <div className="flex items-center gap-4 flex-shrink-0">
          <Link href="/sbu/clearances/finances" className="h-11 px-4 rounded-xl bg-slate-100 border border-slate-200 flex items-center gap-3 hover:bg-white hover:shadow-xl transition-all group/finance">
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="text-left hidden lg:block">
                <p className="text-[10px] font-black text-slate-900 leading-none mb-1 uppercase tracking-tight">{userProfile?.full_name || 'CLEARANCE TEAM'}</p>
                <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none">Dispatcher</p>
              </div>
              ...
              <>
                ...
                <div className="absolute right-0 top-full mt-2 w-64 bg-white border border-slate-200 shadow-2xl rounded-[2rem] p-4 z-50 animate-in zoom-in-95 duration-200 origin-top-right">
                  <div className="p-4 bg-slate-50 rounded-2xl border border-slate-100 mb-2">
                    <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Operational Credits</p>
                    <div className="flex items-center justify-between">
                      <p className="text-xl font-black italic text-slate-900 tracking-tighter uppercase">{tenantInfo.mission_credits || 0} <span className="text-[10px] text-slate-400 not-italic ml-1">TOKENS</span></p>
