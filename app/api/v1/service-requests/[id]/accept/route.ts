...
export async function POST(
  ...
  try {
    ...
    const { tenant_id, domain_job_id } = body;

    if (!tenant_id || !domain_job_id) {
      return NextResponse.json({ success: false, error: 'Missing tenant_id or domain_job_id' }, { status: 400 });
    }

    const result = await service.acceptRequest(id, tenant_id, domain_job_id);
