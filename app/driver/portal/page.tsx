...
export default function DriverPortal() {
  ...
  const [timelinePhotoPreviews, setTimelinePhotoPreviews] = useState<{
  ...
  const [totalCompletedJobsCount, setTotalCompletedJobsCount] =
    useState<number>(0);
  ...
  const handleNativeInstall = async () => {
    ...
    try {
      ...
      toast.error(
        "Gagal membuka pemasang otomatis. Silakan ikuti petunjuk manual di bawah.",
      );
    }
  };
  ...
  const toggleTheme = () => {
    ...
    toast.success(
      `Mode ${nextTheme === "dark" ? "Malam Aktif 🌙" : "Siang Aktif ☀️"}`,
    );
  };
  ...
  const handleCheckOut = async () => {
    ...
    if (
      window.confirm(
        "Yakin ingin Check-Out? Armada akan dikembalikan dan Anda akan berstatus OFF DUTY.",
      )
    ) {
      setLoading(true);
      try {
        ...
        await supabase
          .from("md_drivers")
          .update({ is_working: false, status: "available" })
          .eq("id", driver.id);
        if (activeShift.fleet_id) {
          await supabase
            .from("md_fleets")
            .update({ status: "available" })
            .eq("id", activeShift.fleet_id);
        }
        setActiveShift(null);
        setLastInspection(null);
        toast.success(
          "Berhasil Check-Out. Terima kasih atas kerja keras Anda!",
        );
      } catch (e: any) {
  ...
  useEffect(() => {
    ...
    if (
      (step === "performance" || step === "dashboard" || step === "profile") &&
      driver?.id
    ) {
  ...
  const gpsPingJob = useMemo(
    () =>
      selectedJob ??
      jobOrders.find((jo) => jo.driver_response === "accepted") ??
      null,
  ...
  const fetchAttendanceHistory = async () => {
    ...
    if (data) {
      ...
      if (fleetIds.length > 0) {
        ...
        data.forEach((a) => {
          a.md_fleets = fleetMap.get(a.fleet_id) || null;
        });
      }
    ...
    try {
      ...
      const [woRes, routesRes, paymentsRes] = await Promise.all([
        ...
        joIds.length > 0
          ? supabase
              .from("job_routes")
              .select("job_order_id, distance_km")
      ...
      (paymentsRes.data || []).forEach((p) => {
        if (!paymentsMap.has(p.job_order_id))
          paymentsMap.set(p.job_order_id, []);
      ...
      enrichedJobs = completedJobs.map((j) => {
        ...
        const advancePayments = pMap
          .filter((p: any) => p.payment_type === "advance_driver")
          .reduce((s: number, p: any) => s + Number(p.amount), 0);
        const pelunasanPayments = pMap
        ...
        const legacyAdv =
          j.advance_status === "paid" || j.advance_status === "completed"
            ...
            : 0;
        const legacyPel =
          j.driver_payment_status === "paid" ||
          j.driver_payment_status === "completed"
            ? Math.max(
      ...
      if (allJobs && allJobs.length > 0) {
        ...
        for (const job of enrichedJobs) {
          const routeDist =
            job.job_routes?.reduce(
              (sum: number, r: any) => sum + (Number(r.distance_km) || 0),
              0,
            ) || 0;
        ...
        for (const job of allJobs) {
          ...
          const pelunasanPayments = pMap
            ...
            .reduce((s: number, p: any) => s + Number(p.amount), 0);
          const legacyAdv =
            job.advance_status === "paid" || job.advance_status === "completed"
              ? Number(job.advance_amount || 0)
              : 0;
          const legacyPel =
            job.driver_payment_status === "paid" ||
            job.driver_payment_status === "completed"
              ? Math.max(
                  Number(job.driver_payment_amount || 0),
                  advancePayments > 0 ? advancePayments : legacyAdv,
                )
              : 0;
          ...
          const ch = sp > 0 && bp > 0 ? bp * (sp / 100) : 0;
          const hd =
            Number(job.driver_revenue_share) ||
            ch ||
            Number(job.driver_payment_amount) ||
            Number(job.advance_amount) ||
            0;
  ...
  const fetchJobOrders = async () => {
    ...
    const [woRes, routesRes, fleetsRes, trackingRes] = await Promise.all([
      woItemIds.length > 0
        ? supabase
            .from("wo_items")
            .select("id, item_code, item_data, wo_id")
            .in("id", woItemIds)
        : Promise.resolve({ data: [], error: null }),
      joIds.length > 0
        ? supabase
    ...
    const dataWithJoins = data.map((jo) => ({
      ...
      work_order: jo.wo_item_id
        ? woDetailsMap.get(woMap.get(jo.wo_item_id)?.wo_id)
        : null,
      md_fleets: fleetMap.get(jo.fleet_id) || null,
      job_routes: (routesRes.data || [])
        .filter((r) => r.job_order_id === jo.id)
        .sort((a, b) => a.sequence - b.sequence),
      tracking_logs: (trackingRes.data || []).filter(
        (t) => t.job_order_id === jo.id,
      ),
    }));
  ...
  const fetchInspections = async () => {
    ...
    if (data && data.length > 0) {
      ...
      if (fleetIds.length > 0) {
        ...
        const fleetMap = new Map((fleets || []).map((f) => [f.id, f]));
        data.forEach((i) => {
          i.md_fleets = fleetMap.get(i.fleet_id) || null;
        });
      }
  ...
  const fetchFleets = async () => {
    ...
    const { data, error } = await supabase
      .from("md_fleets")
      .select(
        "id, plate_number, brand, model, status, md_entities(name, is_vendor), md_fleet_types(type_name)",
      )
  ...
  const handleStartShift = async () => {
    ...
    try {
      ...
      await supabase
        ...
        .eq("id", selectedFleetId);
      setActiveShift({
        ...attendance,
        fleet: fleets.find((f) => f.id === selectedFleetId),
      });
    ...
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          tenant_id: driver.tenant_id,
          odometer_value: inspectionData.odometer_value
            ? parseFloat(inspectionData.odometer_value)
            : null,
      ...
      const { data: driverData } = await supabase
        .from("md_drivers")
        .select("total_inspections, avg_inspection_score")
        .eq("id", driver.id)
        .single();
      const newTotalInspections = (driverData?.total_inspections || 0) + 1;
      const newAvgScore =
        ((driverData?.avg_inspection_score || 0) * (newTotalInspections - 1) +
          totalScore) /
        newTotalInspections;
      ...
      if (status === "GROUNDED") {
        toast.error(
          "Kondisi buruk! Truk di-GROUNDED dan tim Ops telah diberi tahu.",
        );
      } else {
  ...
  const handleUpdateJobStatus = async (jobId: string, newStatus: string) => {
    ...
    try {
      ...
      if (newStatus === "START JOURNEY" || newStatus === "IN_PROGRESS")
      ...
      if (apiStatus === "completed") {
        ...
        if (reloadedJob) {
          ...
          const { data: existingRoutes } = await supabase
            .from("job_routes")
            .select("*")
            .eq("job_order_id", reloadedJob.id)
            .order("sequence", { ascending: true });
          let finalRoutes = existingRoutes || [];

          if (
            finalRoutes.length === 0 &&
            reloadedJob?.wo_items?.item_data?.stops
          ) {
          ...
          reloadedJob.routes = (
            await attachRoutePhotos(finalRoutes, reloadedJob.id)
  ...
  const updateRouteStatus = async (routeId: string, routeStatus: string) => {
    ...
    try {
      ...
      if (selectedJobRoutes?.length > 0) {
        const currentIndex = selectedJobRoutes.findIndex(
          (r: any) => r.id === routeId,
        );
        if (currentIndex > 0) {
          ...
          if (prevRoute?.status !== "completed") {
            throw new Error(
              `Selesaikan stop sebelumnya (${prevRoute?.location_name || "-"}) terlebih dahulu`,
            );
          }
        }
      }
      ...
      if (reloadedJob) {
        ...
        const { data: routes } = await supabase
          ...
          .order("sequence", { ascending: true });
        reloadedJob.routes = (
          await attachRoutePhotos(routes || [], reloadedJob.id)
        ).sort((a: any, b: any) => a.sequence - b.sequence);
        setSelectedJob(reloadedJob);
      }
      fetchJobOrders();
  ...
  const handleRoutePhotoUpload = async (
    routeId: string,
  ...
  if (!mounted) {
    return (
      <div
        className={`min-h-screen ${isDark ? "bg-slate-950" : "bg-blue-600"} flex items-center justify-center`}
      >
  ...
  if (step === "profile") {
    return (
      ...
      >
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-8">
            <button
              ...
              className={`w-10 h-10 ${isDark ? "bg-slate-900 border-slate-800 text-slate-300" : "bg-white/20 text-white"} rounded-full flex items-center justify-center border`}
            >
              <X size={20} />
            </button>
            ...
            <button
              onClick={toggleTheme}
              className={`p-2.5 rounded-full border ${isDark ? "bg-slate-900 border-slate-800 text-amber-400" : "bg-white/20 border-white/20 text-white"}`}
            >
          ...
          <div
            className={`rounded-3xl p-6 shadow-xl ${isDark ? "bg-slate-900 border border-slate-850" : "bg-white text-slate-800"}`}
          >
            <div className="flex flex-col items-center mb-6">
              {driver?.photo_url ? (
                ...
                <div
                  className={`w-24 h-24 rounded-full ${isDark ? "bg-slate-950 border-slate-800" : "bg-indigo-50 border-indigo-100"} flex items-center justify-center border-4`}
                >
              ...
              <span
                className={`px-4 py-1.5 rounded-full text-xs font-bold mt-1 uppercase tracking-widest ${driver?.is_working ? "bg-green-100 text-green-700" : "bg-slate-200 text-slate-600"}`}
              >
            ...
            <div className="space-y-4">
              <div
                className={`flex items-center gap-4 p-4 rounded-2xl ${isDark ? "bg-slate-950/50" : "bg-slate-50"}`}
              >
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-xs opacity-70">
                    Exp:{" "}
                    {driver?.sim_expiry
                      ? new Date(driver.sim_expiry).toLocaleDateString("id-ID")
                      : "-"}
                  </p>
                </div>
              </div>
              ...
              {/* 🪙 Coin Balance Card */}
              <div
                className={`flex items-center gap-4 p-4 rounded-2xl ${isDark ? "bg-slate-950/50" : "bg-amber-50 border border-amber-100"}`}
              >
                <div className="w-12 h-12 bg-gradient-to-br from-amber-300 via-yellow-400 to-amber-500 rounded-xl flex items-center justify-center shrink-0 shadow-md">
                  <Coins size={20} className="text-amber-900" />
                </div>
                <div className="flex-1">
                  <p
                    className={`text-xs font-bold uppercase ${isDark ? "text-slate-500" : "text-amber-600"}`}
                  >
                    Saldo Koin
                  </p>
                  <p className="text-2xl font-black text-amber-500">
                    {driver?.total_coins || 0} Koin
                  </p>
                  <p
                    className={`text-xs font-semibold ${isDark ? "text-slate-500" : "text-amber-700"}`}
                  >
                    Rp {(driver?.total_coin_value || 0).toLocaleString("id-ID")}
                  </p>
                </div>
                <span
                  className={`text-[8px] font-black uppercase tracking-widest px-2 py-1 rounded-full ${isDark ? "bg-slate-800 text-amber-400" : "bg-amber-100 text-amber-700"}`}
                >
                  1 Koin = Rp 5.000
                </span>
              </div>
              ...
              <div className="mt-4">
                <div
                  className={`rounded-2xl p-4 text-center ${isDark ? "bg-slate-950" : "bg-blue-50 text-blue-900"}`}
                >
                  <p className="text-2xl font-black text-blue-500">
                    {totalKM > 0
                      ? totalKM.toFixed(0)
                      : Number(driver?.total_km_driven || 0).toFixed(0)}
                  </p>
                  <p
                    className={`text-[10px] font-black uppercase ${isDark ? "text-slate-500" : "text-blue-400"}`}
                  >
                    Total Kilometer Tempuh
                  </p>
                </div>
              </div>
              ...
              {driver?.bank_name && (
                <div className="mt-4 pt-4 border-t border-slate-100">
                  <p
                    className={`text-xs font-bold uppercase mb-2 ${isDark ? "text-slate-500" : "text-slate-400"}`}
                  >
                    Rekening Bank
                  </p>
                  <div
                    className={`rounded-2xl p-4 ${isDark ? "bg-slate-950" : "bg-slate-50"}`}
                  >
        ...
        <nav
          className={`fixed bottom-0 left-0 right-0 p-3 flex justify-around items-center border-t backdrop-blur-lg z-30 ${isDark ? "bg-slate-900/90 border-slate-800 text-slate-400" : "bg-white/95 border-slate-100 shadow-2xl"}`}
        >
          <button
            onClick={() => setStep("dashboard")}
    ...
    return (
      ...
      >
        ...
        <div className="p-6">
          ...
          <div
            className={`rounded-3xl p-6 shadow-xl ${isDark ? "bg-slate-900 border border-slate-850" : "bg-white text-slate-850"}`}
          >
            ...
            {inspectionsList.length === 0 ? (
              <div className="text-center py-8">
                <div
                  className={`w-20 h-20 ${isDark ? "bg-slate-950" : "bg-slate-50"} rounded-full flex items-center justify-center mx-auto mb-4`}
                >
                  <FileCheck size={40} className="text-slate-400" />
                </div>
                <p className="text-lg font-bold">Belum Ada Inspeksi</p>
                <p className="text-sm opacity-60 mt-1">
                  Lakukan inspeksi truk terlebih dahulu
                </p>
              </div>
            ) : (
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-1">
                {inspectionsList.map((insp) => (
                  <div
                    key={insp.id}
                    className={`rounded-2xl p-4 border ${isDark ? "bg-slate-950 border-slate-850" : "bg-slate-50 border-slate-150"}`}
                  >
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <p className="text-sm font-black">
                          {insp.md_fleets?.plate_number || "-"}
                        </p>
                      ...
                      <span
                        className={`px-2 py-0.5 rounded text-[9px] font-black ${insp.status === "LAYAK JALAN" ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                      >
                        {insp.status === "LAYAK JALAN"
                          ? "LAYAK JALAN ✓"
                          : "RUSAK ✕"}
                      </span>
                    </div>
                    <div className="flex justify-between items-center text-xs opacity-75 border-t pt-2 border-slate-200/50">
                      ...
                      <span
                        className={`font-black ${insp.total_score >= 80 ? "text-green-500" : "text-red-500"}`}
                      >
                        {insp.total_score}/100
                      </span>
                    </div>
                  </div>
                ))}
    ...
    ? [
        {
          id: "start",
          label: "TERIMA",
          status: selectedJob.accepted_at ? "completed" : "pending",
        },
        {
          id: "depart",
          label: "BERANGKAT",
          status:
            ...
            (selectedJob.status || "").toUpperCase() === "DALAM PERJALANAN" ||
            (selectedJob.status || "").toUpperCase().startsWith("MENUJU")
              ? "completed"
              : selectedJob.accepted_at ||
                  (selectedJob.status || "").toUpperCase() ===
                    "MENUNGGU MULAI / START" ||
                  (selectedJob.status || "").toUpperCase() === "ORDER DITERIMA"
                ? "current"
                : "pending",
        },
          ...
          status:
            s.status === "completed"
              ...
              : s.status === "arrived" ||
                  (selectedJob.status || "")
                    .toUpperCase()
                    .includes(s.location_name.toUpperCase())
                ? "current"
                : "pending",
        })),
        {
          ...
          status:
            ...
            [
              ...
              "INVOICED",
              "PAID",
            ].includes((selectedJob.status || "").toUpperCase())
              ? "completed"
              : "pending",
        },
      ]
  ...
  return (
    ...
    >
        ...
        <div className="mt-4 bg-white/10 backdrop-blur-md rounded-2xl p-4 border border-white/15 shadow-inner">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center shadow-lg ${activeShift ? "bg-green-500 text-white shadow-green-500/20" : "bg-slate-500/30 text-slate-300"}`}
              >
              ...
              <div>
                ...
                <p className="text-base font-bold mt-0">
                  {activeShift
                    ? "AKTIF BEKERJA (ON DUTY)"
                    : "BELUM ABSEN (OFF DUTY)"}
                </p>
                {activeShift && (
        ...
        {(() => {
          ...
          return (
            <>
                  ...
                  <div className="relative z-10">
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        ...
                        <h3 className="text-xl font-black mt-2 leading-none tracking-tight">
                          {activeJob.jo_number}
                        </h3>
                        <p className="text-xs text-slate-400 mt-1.5 uppercase font-bold tracking-tight">
                          Plat Truk: {activeJob.md_fleets?.plate_number || "-"}
                        </p>
                      </div>
                      <div className="bg-white/10 text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-[0.2em] italic">
                        {(() => {
                          ...
                          if (s === "IN_PROGRESS" || s === "DALAM PERJALANAN")
                    ...
                    <div className="bg-white/5 border border-white/10 rounded-2xl p-4 mb-4">
                      <h4 className="text-[10px] font-black text-indigo-400 uppercase tracking-widest leading-none mb-2">
                        Shipper / Pelanggan
                      </h4>
                      <p className="text-base font-black uppercase italic leading-none">
                        {activeJob.wo_items?.item_data?.shipper_name ||
                          "SENTRALOGIS"}
                      </p>
                      {activeJob.wo_items?.item_data && (
                        <div className="text-xs text-slate-300 font-semibold mt-3 pt-3 border-t border-white/5">
                          🚚{" "}
                          {activeJob.wo_items.item_data.stops?.[0]
                            ?.location_name || "Loading Point"}{" "}
                          →{" "}
                          {activeJob.wo_items.item_data.stops?.[
                            activeJob.wo_items.item_data.stops?.length - 1
                          ]?.location_name || "Unload Point"}
                        </div>
                      )}
                    </div>
              ...
              <div
                className={`rounded-3xl p-5 shadow-xl border ${isDark ? "bg-slate-900 border-slate-800" : "bg-white border-slate-100"}`}
              >
                ...
                {newJobs.length === 0 ? (
                  <div className="py-10 text-center">
                    <div
                      className={`w-16 h-16 ${isDark ? "bg-slate-950" : "bg-slate-50"} rounded-2xl flex items-center justify-center mx-auto mb-3 shadow-inner`}
                    >
                      ...
                      >
                        <div className="flex justify-between items-start mb-3">
                          <div>
                            ...
                            <p className="text-xs opacity-60 mt-0.5">
                              Plat Truk: {jo.md_fleets?.plate_number || "-"}
                            </p>
                          </div>
                          <div className="flex items-center gap-2">
                            <span
                        ...
                        {jo.wo_items?.item_data && (
                          <div className="text-xs opacity-75 font-semibold mt-2 border-t pt-2 border-slate-200/50">
                            🚚{" "}
                            {jo.wo_items.item_data.stops?.[0]?.location_name ||
                              "Loading Point"}{" "}
                            →{" "}
                            {jo.wo_items.item_data.stops?.[
                              jo.wo_items.item_data.stops?.length - 1
                            ]?.location_name || "Unload Point"}
                          </div>
                        )}
                      </div>
      ...
      >
        <button
          onClick={() => setStep("dashboard")}
          className="flex flex-col items-center gap-1 p-2 text-indigo-500"
        >
          <Home size={24} />
          <span className="text-xs font-bold">Home</span>
        </button>
        ...
        >
          ...
          {completedJobsMonth > 0 && (
            <span className="absolute -top-1 -right-1 w-5 h-5 bg-emerald-500 text-white text-[9px] font-black rounded-full flex items-center justify-center shadow">
              {completedJobsMonth}
            </span>
          )}
        </button>
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-[#f8fafc] text-slate-800 font-sans pb-32">
          ...
          <main className="max-w-xl mx-auto px-6 pt-6 space-y-6">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="pr-6">
                    ...
                    <p className="text-xs font-bold text-emerald-100 leading-relaxed">
                      Terverifikasi otomatis via Geofence (Radius{" "}
                      {geofenceBanner.distance_m || "< 500"}m). HP bergetar &
                      status rute telah diperbarui!
                    </p>
                  </div>
                </div>
              </div>
            ...
            {(selectedJob.advance_status === "paid" ||
              selectedJob.advance_status === "completed") && (
              <div className="mb-6 bg-emerald-600 text-white p-5 rounded-[2rem] shadow-xl shadow-emerald-600/20 flex items-center gap-5 animate-in slide-in-from-top-4 duration-700">
                ...
                <div>
                  ...
                  <p className="text-[9px] font-bold text-emerald-100/60 uppercase mt-1">
                    Silakan memulai perjalanan Anda.
                  </p>
                </div>
              </div>
            )}
            ...
            <div className="bg-slate-50 rounded-2xl p-5 border border-slate-100/50 shadow-inner mb-4">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-rose-600 font-black tracking-tight">
                  {formatDate(
                    selectedJob.wo_items?.item_data?.execution_date ||
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="bg-white border border-slate-100 rounded-xl p-3 flex items-center gap-3 shadow-sm">
                ...
                <div className="min-w-0">
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">
                    {selectedJob.wo_items?.item_data?.vehicle_type_name ||
                      "Fleet"}
                  </p>
                  <p className="text-xs font-black text-slate-800 truncate uppercase italic">
                    {selectedJob.md_fleets?.plate_number || "-"}
                  </p>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-xs font-black text-slate-800 uppercase tracking-tight">
                    DOKUMEN PENGANTAR & MANIFEST
                  </h3>
                  <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest">
                    Surat Jalan, Manifest & Instruksi Kerja
                  </p>
                </div>
              </div>

              {selectedJob.assignment_documents &&
              selectedJob.assignment_documents.length > 0 ? (
                <div className="space-y-3 pt-1">
                  {selectedJob.assignment_documents.map(
                    (doc: any, idx: number) => (
                      <div
                        key={doc.id || idx}
                        className="bg-blue-50/50 border border-blue-200/80 rounded-2xl p-4 flex items-center justify-between gap-3 shadow-sm"
                      >
                ...
                <div className="border border-dashed border-blue-200 rounded-2xl p-4 text-center bg-blue-50/30">
                  <p className="text-xs font-bold text-slate-500 mb-1">
                    Belum ada dokumen digital yang dilampirkan admin.
                  </p>
                  <p className="text-[10px] text-slate-400">
                    Surat Jalan fisik / manifest akan diserahkan oleh dispatcher
              ...
              ![
                ...
                <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
                  <h2 className="text-[10px] font-bold text-slate-400 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                    <Activity size={12} className="text-indigo-600" /> JOURNEY
                    PIPELINE
                  </h2>
                  <div className="relative px-2">
                    ...
                    <div
                      ...
                      style={{
                        width:
                          progress > 0 ? `calc(${progress}% - 32px)` : "0px",
                      }}
                    />
                    ...
                    <div className="relative flex justify-between">
                      {milestones.map((m, idx) => (
                        <div key={m.id} className="flex flex-col items-center">
                          <div
                            className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black z-10 transition-all duration-500 shadow-sm ${
                              m.status === "completed"
                                ? "bg-emerald-500 text-white"
                                : m.status === "current"
            ...
            {[
              ...
              <div className="bg-rose-50 border-4 border-rose-500 rounded-[2.5rem] p-8 sm:p-12 text-center shadow-2xl my-8 relative overflow-hidden animate-in fade-in duration-500">
                ...
                <p className="text-xs sm:text-sm font-bold text-slate-600 max-w-md mx-auto mb-6 leading-relaxed">
                  Tugas pengiriman ini telah ditutup dan disahkan selesai.
                  Seluruh akses tindakan supir pada halaman ini telah
                  dikunci/diblokir oleh sistem.
                </p>
                  ...
                  <div className="space-y-4">
                    ...
                    {selectedJob.driver_response !== "accepted" && (
                      <div className="grid grid-cols-2 gap-4">
                        <button
                          onClick={() =>
                            handleUpdateJobStatus(selectedJob.id, "DITERIMA")
                          }
                          disabled={loading}
                          ...
                          onClick={async () => {
                            ...
                            try {
                              let lat = null,
                                lng = null;
                              try {
                                const pos = await new Promise<any>(
                                  (resolve, reject) => {
                        ...
                        >
                          {loading ? (
                            ...
                            <>
                              <Truck size={22} />
                              BERANGKAT MENUJU{" "}
                              {selectedJobRoutes[0]?.location_name?.toUpperCase() ||
                                "LOKASI"}
                            </>
                          )}
                        </button>
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div className="flex gap-4 flex-1">
                            ...
                            <div className="min-w-0 flex-1">
                              ...
                              <h3 className="font-bold text-slate-800 text-base mt-1 uppercase tracking-tight leading-none truncate">
                                {stop.location_name}
                              </h3>
                              <p className="text-[11px] font-medium text-slate-400 leading-relaxed mt-1 break-words">
                                {stop.address}
                              </p>
                              {stop.notes && (
                          ...
                          <div className="flex flex-col gap-2">
                            ...
                            <div className="relative">
                              <input
                                ...
                                id={`photo-${stop.id}`}
                                onChange={(e) =>
                                  handleRoutePhotoUpload(stop.id, e)
                                }
                              />
                              <label
                        ...
                        <div className="mt-6 pt-5 border-t border-slate-100 dark:border-slate-800/50">
                          ...
                          <div className="space-y-3 mb-4 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                            {(selectedJob.tracking_logs || [])
                              .filter(
                                (log: any) =>
                                  log.job_route_id === stop.id ||
                                  log.notes?.includes(stop.id),
                              )
                              .map((log: any, idx: number) => (
                                <div
                                  key={idx}
                                  className="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-slate-800/60 flex flex-col gap-2 relative"
                                >
                                  ...
                                  <p className="text-[11px] font-semibold text-slate-700 dark:text-slate-300 pr-10 leading-relaxed">
                                    {log.notes
                                      ?.replace(
                                        /\[ROUTE:[0-9a-fA-F-]{36}\] /g,
                                        "",
                                      )
                                      .replace(
                                        /Route ID: [0-9a-fA-F-]{36}( \| Catatan: )?(\(Photo Attached\) )?/g,
                                        "",
                                      )
                                      .trim() || "Pembaruan Status"}
                                  </p>
                                  {log.photo_url && (
                                    <div
                                      className="w-20 h-20 rounded-lg overflow-hidden border border-slate-200 cursor-pointer"
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setSelectedPhotoPreview(log.photo_url);
                                      }}
                                    >
                                      <img
                                        src={log.photo_url}
                            ...
                            {(selectedJob.tracking_logs || []).filter(
                              (log: any) =>
                                log.job_route_id === stop.id ||
                                log.notes?.includes(stop.id),
                            ).length === 0 && (
                            ...
                            <div className="flex flex-col gap-2 p-1 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm focus-within:border-blue-400 focus-within:ring-2 focus-within:ring-blue-400/20 transition-all">
                              {timelinePhotoPreviews[stop.id] && (
                                <div className="p-2 relative w-fit">
                                  <img
                                    src={timelinePhotoPreviews[stop.id]!}
                                    alt="Preview"
                                  ...
                                  <button
                                    onClick={() => {
                                      ...
                                      setTimelinePhotoPreviews({
                                        ...timelinePhotoPreviews,
                                        [stop.id]: null,
                                      });
                                    }}
                                  ...
                                  <input
                                    ...
                                    onChange={(e) => {
                                      ...
                                      if (file) {
                                        ...
                                        setTimelinePhotoPreviews({
                                          ...timelinePhotoPreviews,
                                          [stop.id]: URL.createObjectURL(file),
                                        });
                                      }
                                    }}
                                  />
                          ...
                          <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
                            ...
                            {stop.status === "completed" ? (
                              ...
                              <span className="bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 border border-blue-200 dark:border-blue-500/20 px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest flex items-center gap-1.5 animate-pulse">
                                <MapPin size={13} /> Tiba di Titik (Geofence
                                Terverifikasi)
                              </span>
                            ) : (
                        ...
                        <div className="flex items-center gap-3 mb-4">
                          ...
                          <div>
                            ...
                            <p className="text-white/40 text-[10px] font-bold uppercase tracking-tight">
                              Pastikan semua dokumen & foto POD sudah diunggah
                            </p>
                          </div>
                        </div>
                        ...
                        {completedStops < totalStops && (
                          <div className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 mb-6 flex items-center gap-3">
                            ...
                            <p className="text-[10px] font-bold text-amber-200 uppercase leading-tight">
                              Masih ada {totalStops - completedStops} lokasi
                              yang belum ditandai selesai. Lanjutkan?
                            </p>
                          </div>
                        )}
                        ...
                        <button
                          onClick={() => {
                            if (completedStops < totalStops && totalStops > 0) {
                              ...
                              if (
                                !window.confirm(
                                  `⚠️ PERINGATAN: Masih ada ${unvisited} titik lokasi rute yang belum ditandai selesai/terverifikasi oleh satelit.\n\nApakah Anda YAKIN tetap ingin menyelesaikan pekerjaan ini sekarang?`,
                                )
                              ) {
                                return;
                              }
                              ...
                              if (
                                !window.confirm(
                                  "Apakah Anda yakin ingin menyelesaikan seluruh pekerjaan dan menutup job ini?",
                                )
                              ) {
                                return;
                              }
      ...
      {isSOSModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto p-4 flex justify-center items-end sm:items-center bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div
            className={`w-full max-w-md rounded-3xl p-6 shadow-2xl ${isDark ? "bg-slate-900 text-slate-100 border border-slate-800" : "bg-white text-slate-900"}`}
          >
            <div className="text-center mb-6">
              ...
              <p className="text-xs opacity-75 mt-1">
                Pilih kejadian darurat yang sedang Anda alami di lapangan.
              </p>
            </div>
              ...
              <div className="flex gap-3">
                ...
                >
                  {sosLoading ? (
                    ...
                    <>
                      <AlertOctagon size={16} /> Kirim SOS
                    </>
                  )}
                </button>
              </div>
      ...
      {isAttendanceModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div
            className={`w-full max-w-md rounded-3xl p-6 shadow-2xl max-h-[90vh] overflow-y-auto ${isDark ? "bg-slate-900 text-slate-100 border border-slate-850" : "bg-white text-slate-900"}`}
          >
            <div className="text-center mb-6">
              ...
              <p className="text-xs opacity-75 mt-1">
                Pilih plat armada internal yang akan Anda bawa hari ini.
              </p>
              ...
              <div>
                ...
                {fetchingFleets ? (
                  ...
                  <div className="py-8 text-center border-2 border-dashed border-slate-200 rounded-2xl p-4">
                    ...
                    <p className="text-sm font-black">
                      Armada Kosong / Belum Siap
                    </p>
                    <p className="text-xs opacity-60 mt-1">
                      Hubungi HQ Ops untuk mengubah status armada menjadi
                      "available" atau kaitkan dengan entitas Anda.
                    </p>
                  </div>
                ) : (
                    ...
                    {fleets.map((fleet) => (
                      ...
                      >
                        <div>
                          ...
                          <p className="text-xs opacity-60 mt-0.5">
                            {fleet.md_fleet_types?.type_name ||
                              fleet.model ||
                              "-"}
                          </p>
                        </div>
            ...
            <div className="flex gap-3 mt-6">
              ...
              >
                {loading ? (
                  ...
                  <>
                    <CheckCircle size={18} /> Mulai Shift
                  </>
                )}
              </button>
            </div>
      ...
      {isInspectionModalOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm animate-in fade-in">
          <div
            className={`w-full max-w-md rounded-3xl p-5 shadow-2xl max-h-[90vh] overflow-y-auto ${isDark ? "bg-slate-900 text-slate-100 border border-slate-850" : "bg-white text-slate-900"}`}
          >
            <div className="text-center mb-5">
              ...
              <p className="text-xs opacity-75 mt-1">
                Periksa kondisi kelaikan jalan demi keselamatan Anda di jalan.
              </p>
            </div>
            ...
            <div className="space-y-4">
              <div>
                ...
                <div className="flex gap-2">
                  ...
                  />
                  <label
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center cursor-pointer border-2 border-dashed shrink-0 ${
                      inspectionPhotos.odometer
              ...
              {(
                ...
                const notesKey =
                  `${item.key}_notes` as keyof typeof inspectionData;
                return (
                  <div
                    key={item.key}
                    className={`rounded-2xl p-4 border ${isDark ? "bg-slate-950 border-slate-850" : "bg-slate-50 border-slate-200"}`}
                  >
                        ...
                        className={`flex-1 py-3 rounded-xl text-xs font-black transition-all ${
                          inspectionData[okKey]
                            ...
                            : isDark
                              ...
                              : "bg-white border border-slate-350 text-slate-500"
                        }`}
                      >
                        ✓ LAYAK / BAGUS
                      </button>
                      <button
                        ...
                        className={`flex-1 py-3 rounded-xl text-xs font-black transition-all ${
                          inspectionData[okKey] === false
                            ...
                            : isDark
                              ? "bg-slate-900 border border-slate-800 text-slate-400"
                              : "bg-white border border-slate-355 text-slate-500"
                        }`}
                      >
                        ✕ BERMASALAH / RUSAK
                      </button>
                    ...
                    <div className="flex gap-2">
                      ...
                      />
                      <label
                        className={`w-10 h-10 rounded-xl flex items-center justify-center cursor-pointer border border-dashed shrink-0 ${
                          inspectionPhotos[item.key]
                            ? "bg-green-500/20 border-green-500 text-green-500"
                            : isDark
            ...
            <div className="flex gap-3 mt-5">
              ...
              >
                {inspectionLoading ? (
                  ...
                  <>
                    <CheckCircle size={18} /> Simpan Hasil
                  </>
                )}
              </button>
            </div>
      ...
      {step === "history" && (
        <div
          className={`fixed inset-0 z-30 overflow-y-auto pb-28 p-5 space-y-6 ${isDark ? "bg-slate-950 text-slate-100" : "bg-slate-50 text-slate-900"}`}
        >
          <div className="flex items-center justify-between mt-2">
            ...
            <button
              onClick={() => setStep("dashboard")}
              className={`w-10 h-10 ${isDark ? "bg-slate-900 border-slate-800" : "bg-white shadow-sm border border-slate-100"} rounded-full flex items-center justify-center`}
            >
              <X size={20} />
            </button>
          </div>
          ...
          <div
            className={`rounded-3xl p-5 shadow-xl border relative overflow-hidden ${isDark ? "bg-gradient-to-br from-emerald-950/40 via-slate-900 to-slate-900 border-emerald-500/10" : "bg-gradient-to-br from-emerald-600 to-teal-700 text-white"}`}
          >
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                ...
                <h4 className="text-4xl font-black mt-0.5">
                  {completedJobsMonth}
                </h4>
              </div>
            </div>
            <p className="text-xs opacity-75 font-semibold border-t border-white/10 pt-3">
              Total keseluruhan: {totalCompletedJobsCount} JO selesai sepanjang
              karier Anda.
            </p>
          </div>
          ...
          >
            ...
            {performanceLoading ? (
              ...
              <div className="py-8 text-center">
                ...
                <p className="text-xs opacity-60 mt-1">
                  Setelah menyelesaikan tugas, riwayat akan muncul di sini.
                </p>
              </div>
            ) : (
                  ...
                  return (
                    ...
                    >
                      <div className="flex justify-between items-start mb-2">
                        ...
                        <span className="px-2 py-0.5 bg-green-500/10 text-green-500 rounded text-[9px] font-black">
                          SELESAI ✓
                        </span>
                      </div>
                      {job.wo_items?.item_data && (
                        <div className="text-xs opacity-70 font-semibold mb-2">
                          🚚{" "}
                          {job.wo_items.item_data.stops?.[0]?.location_name ||
                            "-"}{" "}
                          →{" "}
                          {job.wo_items.item_data.stops?.[
                            job.wo_items.item_data.stops?.length - 1
                          ]?.location_name || "-"}
                        </div>
                      )}
                      <div className="text-[10px] font-semibold opacity-70 border-t pt-2 border-slate-200/50 space-y-0.5">
                        <div className="flex justify-between">
                          <span>
                            Total Hak: Rp{" "}
                            {Number(job._finances?.hak || 0).toLocaleString(
                              "id-ID",
                            )}
                          </span>
                        ...
                        <div className="flex justify-between">
                          <span>
                            Advance:{" "}
                            {job._finances?.advancePaid > 0
                              ? `Rp ${Number(job._finances.advancePaid).toLocaleString("id-ID")} ✓`
                              : "Pending"}
                          </span>
                          <span>
                            Sisa: Rp{" "}
                            {Number(job._finances?.sisa || 0).toLocaleString(
                              "id-ID",
                            )}
                          </span>
                        </div>
      ...
      {step === "performance" && (
        ...
        >
          ...
          <div className="space-y-4">
            <div
              className={`rounded-3xl p-5 shadow-xl border relative overflow-hidden ${isDark ? "bg-gradient-to-br from-blue-950/40 via-slate-900 to-slate-900 border-blue-500/10" : "bg-gradient-to-br from-blue-700 to-indigo-800 text-white"}`}
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl pointer-events-none" />
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 bg-white/10 backdrop-blur shadow-md rounded-2xl flex items-center justify-center">
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest opacity-80">
                    Total Hak Driver Keseluruhan
                  </p>
                  <h4 className="text-3xl font-black mt-0.5">
                    Rp {totalHak.toLocaleString("id-ID")}
                  </h4>
                </div>
              </div>
              <p className="text-xs opacity-75 mt-1 font-semibold leading-relaxed border-t border-white/10 pt-3">
                *Total hak bagi hasil (base_price × share%) dari seluruh JO yang
                sudah Anda selesaikan.
              </p>
            </div>

            <div
              className={`rounded-3xl p-5 shadow-xl border relative overflow-hidden ${isDark ? "bg-gradient-to-br from-teal-950/40 via-slate-900 to-slate-900 border-teal-500/10" : "bg-gradient-to-br from-teal-600 to-cyan-700 text-white"}`}
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl pointer-events-none" />
              <div className="flex items-center gap-3 mb-4">
                <div className="w-12 h-12 bg-white/10 backdrop-blur shadow-md rounded-2xl flex items-center justify-center">
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest opacity-80">
                    Total Uang Jalan (Advance) Diterima
                  </p>
                  <h4 className="text-3xl font-black mt-0.5">
                    Rp {totalAdvanceReceived.toLocaleString("id-ID")}
                  </h4>
                </div>
              </div>
              <p className="text-xs opacity-75 mt-1 font-semibold leading-relaxed border-t border-white/10 pt-3">
                *Total uang jalan/advance yang sudah ditransfer ke rekening
                Anda.
              </p>
            </div>
            ...
            >
              ...
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="text-xs font-bold uppercase tracking-widest opacity-80">
                    Total Pelunasan Diterima
                  </p>
                  <h4 className="text-3xl font-black mt-0.5">
                    Rp{" "}
                    {(totalEarnings - totalAdvanceReceived).toLocaleString(
                      "id-ID",
                    )}
                  </h4>
                </div>
              ...
              <p className="text-xs opacity-75 mt-1 font-semibold leading-relaxed border-t border-white/10 pt-3">
                *Total pelunasan bagi hasil yang sudah ditransfer ke rekening
                Anda.
              </p>
            </div>

            <div
              className={`rounded-3xl p-5 shadow-xl border relative overflow-hidden ${isDark ? "bg-gradient-to-br from-rose-950/40 via-slate-900 to-slate-900 border-rose-500/10" : "bg-gradient-to-br from-rose-600 to-pink-700 text-white"}`}
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-white/10 rounded-full blur-2xl pointer-events-none" />
              ...
              <p className="text-xs opacity-75 mt-1 font-semibold leading-relaxed border-t border-white/10 pt-3">
                *Total sisa bagi hasil yang belum dicairkan oleh perusahaan ke
                rekening Anda untuk job yang sudah Anda selesaikan.
              </p>
            </div>
          </div>
        </div>
            ...
            {installTab === "android" ? (
              <div className="space-y-4 text-sm leading-relaxed">
                <p className="font-semibold opacity-80 text-xs">
                  Supir pengguna HP Android dapat memasang portal di layar utama
                  dengan mudah:
                </p>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <p>
                    Buka portal ini di browser <strong>Google Chrome</strong> HP
                    Anda.
                  </p>
                </div>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <p>
                    Ketuk tombol <strong>titik tiga (⋮)</strong> di pojok kanan
                    atas Chrome.
                  </p>
                </div>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <p>
                    Ketuk **Instal** atau **Tambah** di layar. Aplikasi langsung
                    terpasang di HP Android Anda!
                  </p>
                </div>
              </div>
            ) : (
              <div className="space-y-4 text-sm leading-relaxed">
                <p className="font-semibold opacity-80 text-xs">
                  Supir pengguna iPhone/iOS dapat memasang portal di layar utama
                  dengan mudah:
                </p>
                ...
                <div className="flex gap-3 items-start">
                  ...
                  <p>
                    Buka portal ini menggunakan browser <strong>Safari</strong>{" "}
                    bawaan iPhone.
                  </p>
                </div>
      ...
      {gpsPingJob &&
        (gpsPingJob.status === "in_progress" ||
          gpsPingJob.status === "DALAM PERJALANAN" ||
          (gpsPingJob.status || "").startsWith("MENUJU") ||
          (gpsPingJob.status || "").startsWith("TIBA")) &&
        step !== "auth" && (
          <button
        ...
        >
          ...
          >
            ...
            <h3 className="text-lg font-black text-center text-slate-900 uppercase tracking-tight mb-1">
              Pusat Sinyal Darurat SOS
            </h3>
            <p className="text-[11px] text-center font-bold text-slate-500 mb-5 leading-relaxed">
              Jawab pertanyaan cepat di bawah. Koordinat GPS &amp; status muatan
              langsung terkirim ke Head Ops Trucking HQ!
            </p>
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const pos: GeolocationPosition | null = await new Promise(
                      (resolve) => {
                    ...
                    const tokenToUse =
                      gpsPingJob.driver_link_token || gpsPingJob.id;
