...
import { getPingTimeStr } from "@/lib/utils/dateUtils";
...
export default function TripReplayModal({
      ...
      jobOrder.tracking_history.forEach((t: any, idx: number) => {
        if (
          ...
          milestones.push({
            ...
            timestamp: getPingTimeStr(t) || new Date().toISOString(),
  ...
  const vehicleMarkerIcon = useMemo(() => {
    const fleetType =
      jobOrder?.fleet_type_name ||
      jobOrder?.fleet?.fleet_type?.type_name ||
      "truck";
  ...
  useEffect(() => {
    ...
    playbackPoints.forEach((p: any) =>
      bounds.extend({ lat: p.lat, lng: p.lng }),
    );
    routeStops.forEach((r: any) =>
      bounds.extend({ lat: Number(r.latitude), lng: Number(r.longitude) }),
    );
            ...
            <div>
              ...
              <p className="text-xs text-slate-400 font-medium">
                ...
                <strong className="text-white">
                  {jobOrder?.jo_number || "TRUCK-MISSION"}
                </strong>{" "}
                • Nopol:{" "}
                <strong className="text-white">
                ...
                • Supir:{" "}
                <strong className="text-white">
                  {jobOrder?.driver_name || jobOrder?.driver?.name || "-"}
                </strong>
              </p>
            </div>
          </div>
          ...
          {playbackPoints.length === 0 ? (
            <div className="flex-1 flex flex-col items-center justify-center text-center p-8">
              ...
              <h3 className="text-base font-black text-white uppercase tracking-wider mb-2">
                Belum Ada Rekaman Ping GPS 10 Detik
              </h3>
              <p className="text-xs text-slate-400 max-w-md">
                Pesanan kerja ini belum memiliki riwayat koordinat GPS
                (`job_tracking`) atau baru saja ditugaskan ke supir.
              </p>
            </div>
          ) : activeTab === "map" ? (
            /* 🗺️ RADAR MAP VIEW */
              ...
              {!isLoaded ? (
                ...
                >
                  ...
                  {playbackPoints[0] && (
                    <Marker
                      position={{
                        lat: playbackPoints[0].lat,
                        lng: playbackPoints[0].lng,
                      }}
                      label={{
                        text: "START",
                        color: "#ffffff",
                        fontWeight: "bold",
                        fontSize: "10px",
                        className: "bg-emerald-600 px-1.5 py-0.5 rounded -mt-8",
                      }}
                      icon={{
                        path:
                          typeof window !== "undefined" && window.google
                  ...
                  {rawMilestones.map((ms: any, idx: number) => (
                    <Marker
                      ...
                      label={{
                        ...
                        className:
                          idx === 0
                            ? "bg-emerald-600 px-1.5 py-0.5 rounded -mt-8 shadow-lg"
                            : idx === rawMilestones.length - 1
                              ? "bg-red-600 px-1.5 py-0.5 rounded -mt-8 shadow-lg"
                              : "bg-amber-600 px-1.5 py-0.5 rounded -mt-8 shadow-lg",
                      }}
                      icon={{
                        path:
                          typeof window !== "undefined" && window.google
                  ...
                  {currentPoint && (
                    <>
                      ...
                      >
                        <div
                          className={`rounded-2xl p-2.5 shadow-2xl text-white min-w-[230px] -translate-x-1/2 -translate-y-[calc(100%+50px)] pointer-events-none backdrop-blur-md relative border ${
                        ...
                        >
                          ...
                          {currentPoint.isMilestone && (
                            <div className="bg-amber-500 text-slate-950 text-[10px] font-black px-2 py-0.5 rounded-lg uppercase mb-1.5 flex items-center justify-between shadow-sm">
                              <span>
                                🏁 MILESTONE #{currentPoint.milestoneNumber}
                              </span>
                              <span className="text-[8px] bg-slate-950/20 px-1.5 py-0.2 rounded font-mono">
                                POINTING
                              </span>
                            </div>
                          )}
                          ...
                          <div className="flex items-center justify-between gap-2 border-b border-slate-800 pb-1.5 mb-1.5">
                            <span
                              className={`text-[10px] font-black flex items-center gap-1 ${currentPoint.isMilestone ? "text-amber-300" : "text-cyan-400"}`}
                            >
                              <Clock size={12} />{" "}
                              {format(
                          ...
                          {(currentPoint.statusUpdate ||
                            currentPoint.notes) && (
                            <div
                              className={`mt-1.5 p-1.5 rounded border text-[9px] font-semibold ${
                                currentPoint.isMilestone
                  ...
                  <tbody className="divide-y divide-slate-800/60">
                    {playbackPoints.map((p: any, idx: number) => (
                      ...
                      >
                        ...
                        <td className="py-3 px-4">
                          <span
                            className={`text-[10px] px-2 py-0.5 rounded font-medium ${p.isMilestone ? "bg-amber-500/20 text-amber-300 border border-amber-500/30 font-bold" : "bg-slate-800 text-slate-300"}`}
                          >
                            {p.statusUpdate}
                          </span>
                        </td>
            ...
            <div className="bg-slate-900 border-t border-slate-800 p-4 shrink-0 z-20 shadow-2xl flex flex-col gap-2.5">
              ...
              {rawMilestones.length > 0 && (
                <div className="flex items-center gap-2 overflow-x-auto pb-1.5 border-b border-slate-800">
                  ...
                  {rawMilestones.map((m: any, idx: number) => {
                    const frameIdx = playbackPoints.findIndex(
                      (p: any) =>
                        p.isMilestone && p.milestoneNumber === idx + 1,
                    );
                    const isReached =
                      currentIndex >= (frameIdx >= 0 ? frameIdx : 0);
                    const isCurrent =
                      frameIdx === currentIndex ||
                      (currentPoint?.isMilestone &&
                        currentPoint.milestoneNumber === idx + 1);
                    return (
                      <button
                        key={m.id || idx}
                        onClick={() => {
                      ...
                      >
                        ...
                        <span className="truncate max-w-[150px] font-mono">
                          {m.statusUpdate || m.stopType}
                        </span>
                        {isCurrent && (
                          <span className="text-[8px] bg-slate-950/20 px-1 py-0.5 rounded font-black uppercase">
                            LIVE
                          </span>
                        )}
                      </button>
              ...
              <div className="flex items-center gap-4 pt-1">
                <span className="text-[10px] font-mono font-bold text-slate-400 min-w-[65px]">
                  {playbackPoints[0]
                    ? format(new Date(playbackPoints[0].timestamp), "HH:mm:ss")
                    : "00:00:00"}
                </span>
                ...
                <span className="text-[10px] font-mono font-bold text-slate-400 min-w-[65px] text-right">
                  {playbackPoints[playbackPoints.length - 1]
                    ? format(
                        new Date(
                          playbackPoints[playbackPoints.length - 1].timestamp,
                        ),
                        "HH:mm:ss",
                      )
                    : "00:00:00"}
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    onClick={() => {
                      setIsPlaying(false);
                      setCurrentIndex((prev) =>
                        Math.min(playbackPoints.length - 1, prev + 1),
                      );
                    }}
                    title="Next Ping (+10s)"
                    className="w-9 h-9 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white flex items-center justify-center transition-all"
