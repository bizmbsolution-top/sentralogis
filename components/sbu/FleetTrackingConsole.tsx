...
import {
  getAdvancedJobCategory,
  JO_DONE_STATUSES,
  JO_REJECTED_STATUSES,
} from "@/lib/domain/jo/status";
...
const UnifiedMissionRadarMap = dynamic(
  () => import("./UnifiedMissionRadarMap"),
  {
    ssr: false,
    loading: () => (
      <div className="h-full w-full bg-slate-950 flex flex-col items-center justify-center border border-slate-800 p-6">
        ...
        <p className="text-slate-300 text-xs font-black uppercase tracking-wider">
          Memuat Radar Videowall...
        </p>
      </div>
    ),
  },
...
const IntelligenceTower = dynamic(() => import("./IntelligenceTower"), {
  ...
  loading: () => (
    <div className="h-full w-full bg-slate-950 flex flex-col items-center justify-center border border-slate-800 p-6">
      <Loader2 className="w-8 h-8 text-blue-500 animate-spin mb-3" />
      <p className="text-slate-300 text-xs font-black uppercase tracking-wider">
        Memuat Intelijen Tower...
      </p>
    </div>
  ),
});

const TripReplayModal = dynamic(() => import("./TripReplayModal"), {
  ssr: false,
});
...
export default function FleetTrackingConsole() {
  ...
  const geofenceTrackerRef = useRef<
    Map<
      string,
      {
        lastGeofence: string;
        unchangedCount: number;
        status: GeofenceMovementStatus;
      }
    >
  >(new Map());
  const [geofenceStatusMap, setGeofenceStatusMap] = useState<
    Record<string, GeofenceMovementStatus>
  >({});
  ...
  const fetchActiveJobs = useCallback(async () => {
    ...
    const isGlobalRole =
      profile?.role === "owner_sentralogis" || profile?.role?.startsWith("hq_");
    ...
    if (!tenantId && isGlobalRole) {
      try {
        ...
        console.error(
          "Failed to resolve fallback tenant ID for FleetTrackingConsole:",
          e,
        );
      }
    }
    ...
    try {
      ...
      const allJosWithAssets = (baseData || []).filter(
        (jo) =>
          jo.driver_id || jo.fleet_id || jo.transporter_id || jo.vendor_id,
      );
      let enrichedAll: any[] = [];
      ...
      if (allJosWithAssets.length > 0) {
        ...
        const driverIds = [
          ...new Set(allJosWithAssets.map((j) => j.driver_id).filter(Boolean)),
        ];
        const fleetIds = [
          ...new Set(allJosWithAssets.map((j) => j.fleet_id).filter(Boolean)),
        ];

        const [driversRes, fleetsRes, routesRes, trackingRes, entitiesRes] =
          (await Promise.all([
            driverIds.length > 0
              ? supabase
                  .from("md_drivers")
            ...
            fleetIds.length > 0
              ? supabase
                  .from("md_fleets")
                  .select(
                    "id, plate_number, fleet_type:md_fleet_types(id, type_name, icon_url)",
                  )
                  .in("id", fleetIds)
              : { data: [] },
            ...
            supabase
              ...
              .in("id", [
                ...new Set(
                  allJosWithAssets
                    .map((j) => j.transporter_id || j.vendor_id)
                    .filter(Boolean),
                ),
              ]),
          ])) as [any, any, any, any, any];

        enrichedAll = (baseData || [])
          .map((jo) => {
            if (
              !jo.driver_id &&
              !jo.fleet_id &&
            ...
            const driver = driversRes.data?.find(
              (d: any) => d.id === jo.driver_id,
            );
            const fleet = fleetsRes.data?.find(
              (f: any) => f.id === jo.fleet_id,
            );
            const transporter = entitiesRes.data?.find(
              (e: any) => e.id === jo.transporter_id || e.id === jo.vendor_id,
            );

            const routes = (routesRes.data || [])
              .filter((r: any) => r.job_order_id === jo.id)
              .sort((a: any, b: any) => a.sequence - b.sequence);

            // Sort by recorded_at (device/phone time) when available, falling back to created_at (server time)
            const tracking = (trackingRes.data || [])
              .filter((t: any) => t.job_order_id === jo.id)
              .sort(
                (a: any, b: any) =>
            ...
            return {
              ...
              plate_number:
                fleet?.plate_number ||
                (transporter ? `VENDOR: ${transporter.name}` : "NO PLATE"),
              driver_name:
                driver?.name || (transporter ? transporter.name : "NO DRIVER"),
      ...
      const activeJOs = enrichedAll.filter((jo) => {
        ...
        const isCompletedPhysically =
          routes.length > 0 &&
          (routes[routes.length - 1].actual_departure != null ||
            routes[routes.length - 1].status === "completed");

        // [FIX] Stale final-stop detection: truck sudah TIBA di lokasi terakhir (DROPOFF)
        // dan GPS sudah berhenti ping > 30 menit => anggap sudah selesai fisik.
        // JO seperti CC-PADI-0826-001-01 (status macet "BERANGKAT DARI LOKASI MUAT",
        // stop terakhir "arrived", ping terakhir 15:11 WIB) harus HILANG dari maps.
        const STALE_MS = 30 * 60 * 1000;
        const lastRoute = routes[routes.length - 1];
        const lastPingTs = jo.latest_log
          ? getPingTimestamp(jo.latest_log)
          : null;
        ...
        return (
          (category === "active" || category === "assigned") &&
          status !== "MENUNGGU SELESAI" &&
          !isCompletedPhysically &&
          !arrivedFinalStopStale
        );
      });

      // Sort job orders by latest ping descending (moving at top, idle at bottom)
      activeJOs.sort((a, b) => {
        const timeA = a.latest_log ? (getPingTimestamp(a.latest_log) ?? 0) : 0;
        const timeB = b.latest_log ? (getPingTimestamp(b.latest_log) ?? 0) : 0;
        return timeB - timeA;
      });
  ...
  useEffect(() => {
    ...
    const checkGeofences = () => {
      ...
      jobOrders.forEach((jo) => {
        ...
        // Get current position - sort by recorded_at (device time) when available, fallback to created_at (server time)
        ...
        if (!existing) {
          ...
          const newStatus: GeofenceMovementStatus =
            newCount >= 10 ? "idle" : "moving";
          tracker.set(jo.id, {
            lastGeofence: currentGeofence,
            unchangedCount: newCount,
            status: newStatus,
          });
  ...
  // KPI Calculations
  const kpis = useMemo(() => {
    const total = visibleJobOrders.length;
    let active = 0;
    let idle = 0;
    let sos = 0;
      ...
      .map((jo) => {
        ...
        if (isSOS) {
          ...
          const diffMins = log
            ...
            : 0;
          const arrivedStop = (jo.routes || []).find(
            (r: any) => r.status === "arrived",
          );
          const idleLocation = arrivedStop
            ? ` @ ${arrivedStop.location_name}`
            : "";
          ...
          const currentStopLabel = arrivedStop
            ? ` @ ${arrivedStop.location_name}`
            : "";
          ...
          if (
            ...
            statusText =
              log?.status_update?.toLowerCase() === "pending" ||
              log?.status_update?.toLowerCase() === "assigned"
                ? "MENUNGGU BERANGKAT"
          ...
          if (log) {
            const diff = Math.max(
              ...
              differenceInMinutes(
                ...
                new Date(
                  jo.updated_at +
                    (jo.updated_at?.includes("+") ||
                    jo.updated_at?.endsWith("Z")
                      ? ""
                      : "Z"),
                ),
              ),
            ...
            const timeStr =
              diff < 60
                ? `${diff} mnt`
                : `${Math.floor(diff / 60)} jam ${diff % 60} mnt`;
            statusText += ` (${timeStr})`;
          }
        ...
        const time = log
          ? format(
              new Date(
                log.created_at +
                  (log.created_at.includes("+") || log.created_at.endsWith("Z")
                    ? ""
                    : "Z"),
              ),
              "HH:mm",
  ...
  return (
    <div
      ref={consoleRef}
      className={`flex ${
        isFullscreen ? "h-screen w-screen" : "h-[calc(100vh-64px)] w-full"
      } bg-slate-950 overflow-hidden relative font-sans`}
    >
      {/* MAIN MAP */}
        ...
        <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-3 md:p-4 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-3 md:gap-4 pointer-events-auto">
          <div className="flex items-center justify-between w-full md:w-auto">
            ...
            <div className="flex items-center gap-2 md:gap-3 md:pr-4">
              ...
              <div>
                <h1 className="text-sm md:text-lg font-black text-white tracking-widest uppercase leading-tight">
                  Control Tower
                </h1>
            ...
            {!joParam && (
              <div className="hidden md:block w-px h-10 bg-slate-700/50 mr-4"></div>
            )}
          ...
          <div className="flex-1 w-full md:w-auto flex flex-wrap md:flex-nowrap items-center justify-around px-2 md:px-4 gap-2">
            ...
            <div className="flex flex-col items-center">
              <span className="text-[10px] font-bold text-rose-400/80 uppercase tracking-widest mb-1">
                Critical / SOS
              </span>
              <div className="flex items-center gap-2">
                <AlertTriangle
          ...
          {visibleJobOrders.length === 0 ? (
            ...
            visibleJobOrders.map((jo) => {
              ...
              return (
                <div
                  key={`${jo.id}`}
                  className={`p-2.5 rounded-lg border flex gap-3 items-start animate-fade-in-up transition-all ${
                    isSOS
                      ? "bg-rose-500/20 border-rose-500/50"
                      : isMoving
                        ? "bg-slate-800/80 border-blue-500/40 shadow-[0_0_10px_rgba(59,130,246,0.1)]"
                  ...
                  <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        ...
                        <span
                          className={`text-[10px] font-medium ${
                            isMoving ? "text-blue-400" : "text-slate-500"
                          }`}
                        >
                          {format(
                            new Date(
                    ...
                    >
                      {(() => {
                        ...
                        const stopLabel = arrivedStop
                          ? ` @ ${arrivedStop.location_name}`
                          : "";
                        const eventLabel = log.status_update?.includes(
                          "Geofence",
                        )
                          ? log.status_update
                          : null;
                        ...
                        if (
                          ...
                          if (s === "completed" || s === "PEKERJAAN SELESAI")
                            return "SELESAI";
                          if (s.startsWith("TIBA"))
                            return s.toUpperCase() + stopLabel;
                        ...
                        return log?.status_update?.toLowerCase() ===
                          "pending" ||
                          log?.status_update?.toLowerCase() === "assigned"
                          ? "MENUNGGU BERANGKAT"
                          : log?.status_update || s?.toUpperCase() || "AKTIF";
                    ...
                    {!isMoving && !isSOS && (
                      <p className="text-[9px] text-slate-500 mt-1 italic">
                        Idle / Tidak ada pergerakan ({diffMins} mnt)
                      </p>
                    )}
                    ...
                    <div className="flex flex-wrap items-center gap-1.5 mt-2 pt-2 border-t border-slate-700/50">
                      ...
                      <span className="text-[9px] font-bold px-1.5 py-0.5 rounded bg-indigo-500/20 text-indigo-300 border border-indigo-500/30">
                        {jo.job_status
                          ? jo.job_status.replace(/_/g, " ").toUpperCase()
                          : "UNKNOWN"}
                      </span>
