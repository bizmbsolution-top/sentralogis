...
export default function SBUFinanceHybridModal({ job, onClose, onSuccess }: SBUFinanceHybridModalProps) {
  const [loading, setLoading] = useState(false);
  const [submitting, setSubmitting] = useState(false);
  const isLocked = ['invoiced', 'paid', 'INVOICED', 'PAID'].includes(job.status);
  ...
  const [docs, setDocs] = useState<DocRow[]>([
    { id: Math.random().toString(), name: '', description: '', file: null, url: null }
  ]);
  ...
  const fetchJobDetails = async () => {
    try {
      ...
      if (data) {
        ...
        setTransferProof(data.advance_receipt_url || data.transfer_proof_url || null);
        ...
        if (data.pod_photo_url) {
          try {
            ...
            if (Array.isArray(urls)) {
              setDocs(urls.map((url: string, index: number) => ({
                id: `existing-${index}`,
            ...
            if (data.pod_photo_url.length > 5) {
               setDocs([{ id: 'existing-0', name: 'Existing Document', description: '', file: null, url: data.pod_photo_url }]);
            }
          }
        }
      }
  ...
  const fetchExistingCosts = async () => {
    try {
      ...
      if (data && data.length > 0) {
        ...
        setExtraCosts([{ id: `new-${Date.now()}`, name: '', amount: '', paid_by_entity: 'internal', status: 'draft' }]);
      }
    } catch (err) {
  ...
  const handleDocUpload = async (id: string, file: File) => {
    try {
      ...
      setDocs(prev => prev.map(d => d.id === id ? { ...d, url: publicUrl, file } : d));
  ...
  const totalExtra = extraCosts.reduce((acc, c) => acc + (Number(parseThousand(c.amount)) || 0), 0);
  
  const targetAmount = job.md_fleets?.md_entities?.is_vendor ? (job.purchase_price || 0) : (job.advance_amount || 0);
  const remainingBalance = targetAmount - (Number(parseThousand(transferAmount)) || 0);

  const handleCompleteDocs = async () => {
    try {
      setLoading(true);
      const validDocs = docs.filter(d => d.url);
      if (validDocs.length === 0) return toast.error('Harap upload setidaknya satu dokumen');
  ...
  const handleSaveDraft = async () => {
    try {
      ...
      if (validDocs.length > 0) {
          updatePayload.pod_photo_url = JSON.stringify(validDocs.map(d => d.url));
      }
      ...
      const validExtra = extraCosts.filter(c => (c.name && c.name.trim() !== '') && c.amount);
      ...
      for (const cost of validExtra) {
          ...
          if (cost.id.includes('new-')) {
              ...
              result = await supabase.from('extra_costs').update(payload).eq('id', cost.id);
          }
          
          if (result.error) {
            console.error('Extra Cost DB Error:', result.error);
            throw new Error(`Gagal simpan biaya "${cost.name}": ${result.error.message}`);
          }
      }

      toast.success('Draft berhasil disimpan');
      // Non-blocking refresh
      refreshData().catch(err => console.error('Silent refresh failed:', err));
    } catch (err: any) {
    ...
    try {
      ...
      for (const cost of validExtra) {
          ...
          if (cost.id.includes('new-')) {
              ...
              res = await supabase.from('extra_costs').update(payload).eq('id', cost.id);
          }
      ...
      if (validDocs.length > 0) {
        const urls = validDocs.map(d => d.url);
        const { error: docErr } = await supabase.from('job_orders').update({ pod_photo_url: JSON.stringify(urls) }).eq('id', job.id);
        if (docErr) throw docErr;
      }

      // 4. Update Status to Live Mission if proof uploaded
      if (transferProof && (job.status === 'accepted' || job.status === 'ORDER DITERIMA')) {
          const { error: statErr } = await supabase.from('job_orders').update({ status: 'MENUNGGU BERANGKAT' }).eq('id', job.id);
      ...
      toast.error('Gagal submit: ' + (err.message || 'Terjadi kesalahan sistem'));
    } finally {
      setSubmitting(false);
    }
  };
  ...
  const handleDeleteCost = async (costId: string) => {
    ...
    try {
      const { error } = await supabase.from('extra_costs').delete().eq('id', costId);
      if (error) throw error;
      setExtraCosts(prev => prev.filter(c => c.id !== costId));
      ...
      <div className="bg-white rounded-[3rem] w-full max-w-6xl max-h-[95vh] overflow-hidden shadow-2xl flex flex-col animate-in zoom-in-95 duration-300">
        ...
        <div className="bg-slate-900 px-10 py-8 flex justify-between items-center shrink-0 border-b border-white/5">
          <div className="flex items-center gap-5">
            ...
            <div>
              <h3 className="text-white font-black text-2xl italic uppercase tracking-tighter leading-none">Finances Hybrid Hub</h3>
          ...
          <button onClick={onClose} className="w-12 h-12 bg-white/10 rounded-full flex items-center justify-center text-slate-400 hover:text-white hover:bg-rose-500 transition-all">
            <X size={24} />
          </button>
        </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
             <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 group hover:border-blue-500/20 transition-all">
                ...
                <div className="space-y-4">
                   <div>
                      <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Job Order ID</p>
                      <p className="text-sm font-black text-slate-900 truncate">{job.jo_number}</p>
                   </div>
                   <div>
                      <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Customer (Pelanggan)</p>
                      <p className="text-sm font-black text-slate-900 truncate">{job.wo_items?.work_orders?.md_entities?.name || 'DIRECT CUSTOMER'}</p>
                   </div>
                </div>
             </div>
             ...
             <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 group hover:border-blue-500/20 transition-all">
                ...
                <div className="space-y-4">
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                         <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Execution Date</p>
                         <p className="text-sm font-black text-slate-900">{job.wo_items?.work_orders?.execution_date ? new Date(job.wo_items.work_orders.execution_date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'}</p>
                      </div>
                      <div>
                         <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Truck Type</p>
                         <p className="text-sm font-black text-slate-900">{job.md_fleets?.md_fleet_types?.type_name || job.md_fleets?.truck_type || '-'}</p>
                      </div>
                   </div>
                   <div>
                      <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Transporter</p>
                      <p className="text-sm font-black text-slate-900 truncate">{job.md_fleets?.md_entities?.name || 'INTERNAL SBU'}</p>
                   </div>
                </div>
             </div>
             ...
             <div className="bg-slate-50 rounded-3xl p-6 border border-slate-100 group hover:border-blue-500/20 transition-all">
                ...
                <div className="flex items-center gap-4 h-full pb-6">
                   <div className="flex-1">
                      <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Origin</p>
                      <p className="text-xs font-black text-slate-900 uppercase truncate">{job.wo_items?.item_data?.origin?.name || 'HUB SBU'}</p>
                   </div>
                   <ArrowRight size={16} className="text-slate-300" />
                   <div className="flex-1">
                      <p className="text-[7px] font-black text-slate-400 uppercase tracking-tighter">Destination</p>
                      <p className="text-xs font-black text-slate-900 uppercase truncate">{job.wo_items?.item_data?.destination?.name || 'DESTINATION'}</p>
                   </div>
                </div>
             </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
             ...
             <div className="space-y-6">
                <div className="flex items-center gap-3">
                   ...
                   <div>
                      <h4 className="text-sm font-black text-slate-900 uppercase tracking-tighter italic">Part 1: Cash Advances</h4>
                      <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Settlement by SBU Finance</p>
                   </div>
                </div>
                ...
                <div className="bg-white border-2 border-slate-100 rounded-[2.5rem] p-8 space-y-8 shadow-sm">
                   <div className="grid grid-cols-2 gap-8">
                      ...
                      <div>
                         ...
                         <div className="mt-2 flex items-center gap-2">
                            <span className="text-[8px] font-black text-slate-400 uppercase">Remaining:</span>
                            <span className={`text-[10px] font-black uppercase ${remainingBalance > 0 ? 'text-rose-500' : 'text-emerald-500'}`}>
                   ...
                   <div className="space-y-3">
                      <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Nominal Transfer</label>
                      <Input 
                         ...
                         onChange={(e) => setTransferAmount(parseThousand(e.target.value))}
                      />
                   </div>

                   <div className="space-y-3">
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest px-1">Proof of Transfer</p>
                      {transferProof ? (
                         <div className="h-48 rounded-3xl border-2 border-dashed border-emerald-200 bg-emerald-50 flex flex-col items-center justify-center group relative overflow-hidden">
                            ...
                            <p className="text-[10px] font-black text-emerald-600 uppercase tracking-widest">Receipt Uploaded</p>
                            <button 
                            ...
                            >
                               <Trash2 size={32} />
                               <span className="text-[10px] font-black uppercase tracking-widest mt-2">Replace File</span>
                            </button>
                         </div>
                      ) : (
                         <div className="relative">
                            ...
                            >
                               <div className="w-16 h-16 bg-white rounded-2xl flex items-center justify-center shadow-sm mb-4 group-hover:scale-110 transition-transform">
                                  {uploadingProof ? <Loader2 className="animate-spin text-blue-500" /> : <Upload size={32} className="text-slate-300" />}
                               </div>
                               <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Select Image / PDF</p>
                            </label>
                         </div>
                      )}
                   </div>
             ...
             <div className="space-y-6">
                <div className="flex items-center justify-between">
                   <div className="flex items-center gap-3">
                      ...
                      <div>
                         <h4 className="text-sm font-black text-slate-900 uppercase tracking-tighter italic">Part 2: Add Cost & Billing Audit</h4>
                         <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Requires CS Approval for Invoicing</p>
                      </div>
                   </div>
                   <Button 
                      variant="ghost" 
                      onClick={() => setExtraCosts([...extraCosts, { id: `new-${Date.now()}`, name: '', amount: '', paid_by_entity: 'internal' }])}
                ...
                <div className="space-y-4">
                   {extraCosts.map((cost) => (
                      <div key={cost.id} className="bg-white border-2 border-slate-100 rounded-3xl p-6 shadow-sm flex items-center gap-4 relative overflow-hidden group">
                         {/* Status Ribbon/Badge */}
                         {cost.status && cost.status !== 'draft' && (
                            <div className={`absolute top-0 right-0 px-4 py-1 text-[7px] font-black uppercase tracking-tighter italic ${
                               cost.status === 'approved' ? 'bg-emerald-500 text-white' : 
                               cost.status === 'need_approval' ? 'bg-amber-400 text-white animate-pulse' : 
                               'bg-rose-500 text-white'
                            }`}>
                               {cost.status === 'need_approval' ? 'Waiting CS' : cost.status}
                            </div>
                         )}
                         ...
                         <div className="flex-1 space-y-4">
                            <Input 
                               ...
                               onChange={(e) => {
                                 ...
                                 setExtraCosts(prev => prev.map(c => c.id === cost.id ? { ...c, name: newVal } : c));
                               }}
                            />
                            <div className="flex items-center gap-3">
                               <Input 
                                  ...
                                  onChange={(e) => {
                                    ...
                                    setExtraCosts(prev => prev.map(c => c.id === cost.id ? { ...c, amount: newVal } : c));
                                  }}
                               />
                               <select 
                                  className={`h-12 px-3 rounded-xl text-[9px] font-black uppercase outline-none border-2 ${
                                     cost.paid_by_entity === 'internal' ? 'bg-blue-50 border-blue-100 text-blue-600' : 'bg-orange-50 border-orange-100 text-orange-600'
                                  }`}
                                  value={cost.paid_by_entity}
                                  disabled={cost.status === 'approved'}
                                  onChange={(e) => setExtraCosts(prev => prev.map(c => c.id === cost.id ? { ...c, paid_by_entity: e.target.value as any } : c))}
                               >
                         ...
                         <div className="flex flex-col gap-2">
                            <button 
                               ...
                               disabled={cost.status === 'approved' || (cost.status === 'rejected_as_cogs' && cost.paid_by_sbu)}
                               ...
                               title={cost.status === 'approved' ? "Approved items cannot be deleted" : "Delete Item"}
                            >
                               <Trash2 size={18} />
                            </button>
                         </div>
                   ...
                   <div className="bg-slate-900 rounded-[2.5rem] p-8 text-white flex justify-between items-center shadow-xl shadow-slate-900/20">
                      <div>
                         <p className="text-[10px] font-black text-white/40 uppercase tracking-widest mb-1 italic">Total Extra Costs</p>
                         <p className="text-2xl font-black text-white tracking-tighter italic">{formatRupiah(totalExtra)}</p>
                      </div>
                ...
                <div className="space-y-6 pt-12">
                   <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                         ...
                         <div>
                            <h4 className="text-sm font-black text-slate-900 uppercase tracking-tighter italic">Part 3: Documents & Finalization</h4>
                            <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">Settlement by Ops SBU (Submit to HQ)</p>
                         </div>
                      </div>
                      <Button 
                         variant="ghost" 
                         onClick={() => setDocs([...docs, { id: Math.random().toString(), name: '', description: '', file: null, url: null }])}
                         className="text-blue-600 font-black text-[10px] uppercase tracking-widest hover:bg-blue-50"
                      >
                   ...
                   <div className="space-y-4">
                      {docs.map((doc) => (
                         <div key={doc.id} className="bg-white border-2 border-slate-100 rounded-3xl p-6 shadow-sm flex items-center gap-4">
                               ...
                               <div className="flex items-center gap-2">
                                  ...
                                  <label 
                                     ...
                                     className={`flex-1 h-10 rounded-xl flex items-center justify-center gap-2 text-[9px] font-black uppercase tracking-widest cursor-pointer transition-all border-2 border-dashed ${
                                        doc.url ? 'bg-emerald-50 border-emerald-200 text-emerald-600' : 'bg-slate-50 border-slate-100 text-slate-400 hover:border-blue-400'
                                     }`}
                                  >
                                     {doc.url ? <><FileCheck size={14} /> Ready</> : <><Upload size={14} /> Upload</>}
                                  </label>
                               </div>
                            </div>
                            <button 
